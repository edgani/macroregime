...
def main() -> int:
    ...
    check("CLI identifies V8.8", "WAR ROOM OS V8.8" in run_text and "War Room OS V8.8" in run_text)
    check("Windows operator scripts present", all((HERE / name).exists() for name in ("SETUP_V88.bat", "CHECK_V88.bat", "RUN_V88.bat")))
    readme = (HERE / "README.md").read_text(encoding="utf-8")
    check("README preserves capital-blocked claim limit", "not" in readme.lower() and "cannot authorize capital" in readme and "zero-technical-indicator" in readme)
    check("dashboard identifies V8.8", "V8.8" in html and "PRICE PROJECTION" in html)
