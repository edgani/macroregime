"""Prospective shadow-order recorder for eligible V10.0 packets."""
...
LEDGER = HERE / "runtime" / "v100_shadow" / "shadow_ledger.jsonl"
UTC = dt.timezone.utc
...
def record(snapshot: Mapping[str, Any] | None = None, *, max_new: int = 12, simulate_fill: bool = True) -> dict[str, Any]:
    ...
    code_hash=_file_hash(HERE/"action_engine_v100.py")
    model_hash=_file_hash(HERE/"V100_ACTION_POLICY.json")
    trial_hash=_file_hash(HERE/"V100_ACTION_POLICY.json")
    for packet in candidates[:max_new]:
        ...
        day=now.strftime("%Y%m%d"); fid=(f"F95_V100_{market.upper()}_{ticker.upper().replace('=','_').replace('.','_')}_{day}")[:84]
        if fid in existing:
        ...
        forecast={
            "forecast_id":fid,"trial_id":"V100_FIXED_ACTION_POLICY","market":market,"security_id":ticker,
    ...
    result={"schema":"warroom.v100.shadow_run.v1","generated_at":now.isoformat().replace("+00:00","Z"),"created":len(created),"created_rows":created,"skipped":skipped,"verification":verify(LEDGER),"ledger":str(LEDGER.relative_to(HERE)),"claim_limit":"Prospective shadow simulation only; not live performance."}
    out=HERE/"runtime"/"v100_shadow"/"last_run.json";out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(result,indent=2,ensure_ascii=False),encoding="utf-8")
    return result
