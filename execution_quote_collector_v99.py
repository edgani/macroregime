"""Resilient current execution-reference quote collector for War Room OS V9.9.
...
UNIVERSE_PATH = HERE / "V99_EXECUTION_REFERENCE_UNIVERSE.json"
DEFAULT_OUT = HERE / "runtime" / "v99_trading" / "execution_quotes.json"
UTC = dt.timezone.utc
USER_AGENT = "WarRoomOS/9.9 execution-reference-only"
...
def collect(*, universe_path: Path = UNIVERSE_PATH, output_path: Path = DEFAULT_OUT) -> dict[str, Any]:
    ...
    payload = {
        "schema": "warroom.v99.execution_quotes.v1", "generated_at": _iso(received_at),
