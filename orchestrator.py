"""orchestrator.py — MacroRegime Pro Orchestrator v3.1 PATCHED
Integrates: GIP + MarketHealth + Real Options + COT + DeFiLlama + CME +
             Forward-Looking (LeadingIndicator + RegimePredictor) +
             PriceCluster + NewsNLP + AutoDiscoveryV3 + BottleneckDiscoveryV3 +
             FrontRunEngine + IHSG Structural Compensators
"""
...
# ── NARRATIVE UNIVERSE (lazy) ──
NARRATIVE_UNIVERSE = {}

def _load_narrative_universe():
    global NARRATIVE_UNIVERSE
    if not NARRATIVE_UNIVERSE:
        try:
            from config import narrative_universe as nu_mod
            NARRATIVE_UNIVERSE = getattr(nu_mod, 'NARRATIVE_UNIVERSE', {})
        except Exception:
            NARRATIVE_UNIVERSE = {}
    return NARRATIVE_UNIVERSE
...
try:
    ...
    logger.warning(f"frontrun_engine import failed: {_e}")
    class FrontRunEngine:
        def run(self, snapshot): return {"boarding_now": [], "gate_opens_soon": [], "check_in": [], "wait": []}
...
def _enrich_narrative_from_universe(narratives, sq):
    nu = _load_narrative_universe()
    if not nu: return narratives
    ...
    for nar_name, nar_data in nu.items():
        if not isinstance(nar_data, dict): continue
        ...
        if str(nar_name).lower() in existing_names: continue
        narratives.append({
            "name": str(nar_name),
...
# ══════════════════════════════════════════════════════════════════════════════
# MAIN BUILD SNAPSHOT — PATCHED v3.1
# ══════════════════════════════════════════════════════════════════════════════
...
def build_snapshot(progress_cb=None, include_us_stocks=True, include_forex=True,
    ...
    try:
        ...
        for c in bottleneck_v3_result.get("reactive", []):
        ...
        for c in bottleneck_v3_result.get("proactive", []):
            mapped = {
                "ticker": c.get("ticker", "UNKNOWN"),
                "direction": "LONG",
                "sector": c.get("sector", "Macro"),
            ...
            auto_discoveries.setdefault("bottlenecks", []).append(mapped)
        if progress_cb: progress_cb(f"Bottleneck V3: {bottleneck_v3_result['meta'].get('reactive_found', 0)} reactive, {bottleneck_v3_result['meta'].get('proactive_predicted', 0)} proactive, {bottleneck_v3_result['meta'].get('spillover_links', 0)} spillover", 0.82)
    except Exception as _disc_err:
    ...
    # ── BUILD ALPHA IDEAS BEFORE AI ENGINE ──
    # FIX: alpha must exist before AI Engine block references it
    gamma_data = {}
    greeks_data = {}
    try:
    ...
    if defillama:
        ...
        crypto_onchain = {"ok": False, "source": "DeFiLlama (not initialized)"}
    ...
    # ── FRONTRUN ENGINE (integrated) ──
    if progress_cb: progress_cb("Running FrontRun Engine...", 0.88)
    frontrun_result = {"boarding_now": [], "gate_opens_soon": [], "check_in": [], "wait": []}
    try:
        fr_engine = FrontRunEngine()
        frontrun_result = fr_engine.run({
        ...
        if progress_cb: progress_cb(f"FrontRun: {len(frontrun_result.get('boarding_now',[]))} boarding", 0.89)
    except Exception as _fr_err:
        logger.warning(f"FrontRunEngine error: {_fr_err}")
        # fallback: use alpha_center frontrun fields
        frontrun_result = {
    ...
    if dxy_s is not None and len(dxy_s) >= 22:
        ...
        dxy_ret = dxy_s.pct_change().dropna()
        for ticker, s in prices.items():
            if s is None or len(s) < 22 or ticker == "DX-Y.NYB":
                continue
            s = pd.to_numeric(s, errors="coerce").dropna()
            if len(s) < 22: continue
            ticker_ret = s.pct_change().dropna()
            combined = pd.concat([dxy_ret, ticker_ret], axis=1, join="inner").dropna()
            if len(combined) >= 20:
                try:
                    c = float(combined.iloc[:,0].tail(20).corr(combined.iloc[:,1].tail(20)))
                    if math.isfinite(c):
                        dxy_corr[ticker] = {
                            "correlation": round(c, 3),
                            "direction": "POSITIVE" if c > 0.3 else ("NEGATIVE" if c < -0.3 else "NEUTRAL"),
                            "strength": "STRONG" if abs(c) > 0.6 else ("MODERATE" if abs(c) > 0.3 else "WEAK"),
                        }
                except Exception: pass
    dxy_summary = {
    ...
    snapshot = {
        ...
        "prices": prices,
        "alpha": alpha,
        "crypto_setups": _build_crypto_setups(prices),
        ...
        "bottleneck_discovery": bottleneck_v3_result,   # ← FIX: separate key, not overwritten
        "frontrun": frontrun_result,
        "frontrun_summary": frontrun_result,
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        ...
        "discovery_candidates": snap.get("auto_discovery", {}).get("meta", {}).get("total_candidates", 0),
        ...
        "bottleneck_v3_reactive": snap.get("bottleneck_discovery", {}).get("meta", {}).get("reactive_found", 0),
        "bottleneck_v3_proactive": snap.get("bottleneck_discovery", {}).get("meta", {}).get("proactive_predicted", 0),
        "bottleneck_v3_spillover": snap.get("bottleneck_discovery", {}).get("meta", {}).get("spillover_links", 0),
        "frontrun_boarding": len(snap.get("frontrun", {}).get("boarding_now", [])),
        "frontrun_gate": len(snap.get("frontrun", {}).get("gate_opens_soon", [])),
        "frontrun_checkin": len(snap.get("frontrun", {}).get("check_in", [])),
