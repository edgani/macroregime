"""Current non-technical market context for War Room OS V10.0.
...
ROOT = HERE / "runtime" / "v100_current"
UTC = dt.timezone.utc
USER_AGENT = "WarRoomOS/10.0 current-context; research-and-execution-reference-only"
...
def collect_quotes(output: Path = ROOT / "quotes.json", *, max_symbols: int | None = None) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema": "warroom.v100.current_quotes.v1", "generated_at": iso(collected_at), "markets": results,
...
def collect_macro(output: Path = ROOT / "macro.json") -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema": "warroom.v100.current_macro.v1", "generated_at": iso(), "series": dict(sorted(rows.items())),
...
def collect_fundamentals(output: Path = ROOT / "fundamentals.json", *, max_equities: int | None = None) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema": "warroom.v100.current_fundamentals.v1", "generated_at": iso(), "markets": markets,
...
def collect_crypto_network(output: Path = ROOT / "crypto_network.json") -> dict[str, Any]:
    ...
    try:
        ...
        out: dict[str, Any] = {"schema": "warroom.v100.crypto_network.v1", "generated_at": iso(), "assets": result, "record_count": len(result), "failures": [], "claim_limit": "Network activity and value capture are research inputs; venue leverage and unlock data are separate."}
    except Exception as exc:
        ...
        out = {"schema": "warroom.v100.crypto_network.v1", "generated_at": iso(), "assets": result, "record_count": len(result), "failures": [{"error": f"{type(exc).__name__}: {exc}"}], "claim_limit": "Network collector unavailable; last-known records, if any, remain stale research context."}
...
def collect_positioning(output: Path = ROOT / "positioning.json") -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {"schema": "warroom.v100.positioning.v1", "generated_at": iso(), "datasets": datasets, "failures": failures, "claim_limit": "Release-lagged positioning is an amplification/context input, never a standalone signal."}
    payload["manifest_hash"] = digest({k: v for k, v in payload.items() if k != "manifest_hash"}); atomic_json(output, payload); return payload
...
def collect_all(*, fast: bool = False) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema": "warroom.v100.current_context_manifest.v1", "generated_at": iso(),
        "paths": {"quotes": "runtime/v100_current/quotes.json", "macro": "runtime/v100_current/macro.json", "fundamentals": "runtime/v100_current/fundamentals.json", "crypto_network": "runtime/v100_current/crypto_network.json", "positioning": "runtime/v100_current/positioning.json"},
        "counts": {"quotes": quotes.get("quote_count", 0), "fresh_quotes": quotes.get("fresh_quote_count", 0), "macro": macro.get("series_count", 0), "fundamentals": fundamentals.get("record_count", 0), "crypto_network": crypto.get("record_count", 0), "positioning_datasets": len(positioning.get("datasets") or {})},
...
def main() -> None:
    ...
    parser.add_argument("command", choices=["all", "fast", "quotes", "macro", "fundamentals", "crypto", "positioning", "show"])
    args = parser.parse_args()
    mapping = {"all": lambda: collect_all(fast=False), "fast": lambda: collect_all(fast=True), "quotes": collect_quotes, "macro": collect_macro, "fundamentals": collect_fundamentals, "crypto": collect_crypto_network, "positioning": collect_positioning, "show": load_all}
