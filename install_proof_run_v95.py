"""Install a passed, signed, hash-bound V9.5 market proof run into the dashboard registry."""
...
REGISTRY=HERE/'component_registry_v95.json'
...
def install(market:str,proof_run:Path)->dict[str,Any]:
    ...
    reasons=[]
    if run.get('schema')!='warroom.v95.blind_proof_run.v1':reasons.append('wrong schema')
    ...
    target=HERE/'runtime'/'v95_proof_runs'/f'{market}.json';target.parent.mkdir(parents=True,exist_ok=True)
    ...
    registry=json.loads(REGISTRY.read_text(encoding='utf-8'));component=f'{market}_bottleneck_price_projection_v95'
    ...
    return {'schema':'warroom.v95.proof_run_install.v1','market':market,'component':component,'path':target.relative_to(HERE).as_posix(),'sha256':digest,'installed':True,'capital_permission':'PENDING_GLOBAL_REGISTRY_ADJUDICATION'}
