"""Adversarial validation for V9.8 limited-production trading controls."""
...
import execution_reconciliation_v98 as XR
import trading_control_plane_v98 as TC
from trading_readiness_v98 import audit as readiness_audit
...
def quote_payload(now: dt.datetime, *, price: float = 500.0) -> dict[str, Any]:
    ...
    payload = {
        "schema": "warroom.v98.execution_quotes.v1",
...
def account_payload(now: dt.datetime) -> dict[str, Any]:
    return {
        "schema": "warroom.v98.account_state.v1",
...
def decision_payload(now: dt.datetime, proof_path: str, proof_hash: str) -> dict[str, Any]:
    return {
        "schema": "warroom.v98.trade_decision.v1",
...
def main() -> dict[str, Any]:
    ...
    fixture_root = HERE / "runtime" / "v98_validation_fixture"
    ...
    try:
        ...
        proof = {
            "schema": "warroom.v98.blind_proof_run.v1", "market": "us", "trading_ready": True,
        ...
        check("policy schema and auto-submit disabled", lambda: policy["schema"] == "warroom.v98.limited_production_policy.v1" and policy["execution_rules"]["auto_submit_enabled"] is False, results)
        ...
        TC.release_kill_switch(actor="validator", explicit_text="RELEASE V98 KILL SWITCH")
        base = TC.evaluate(decision, account, quotes, now=now)
    ...
    payload = {
        "schema": "warroom.v98.final_validation.v1",
    ...
    (HERE / "V98_FINAL_VALIDATION.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
