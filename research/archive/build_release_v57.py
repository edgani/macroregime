"""Build byte-deterministic War Room OS v5.7 release."""
from __future__ import annotations
import hashlib,json,tempfile,zipfile
from pathlib import Path
from verify_manifest_v57 import ROOT,MANIFEST,release_files,verify
OUT=ROOT.parent/'War_Room_OS_v57_Cusp_Falsification_Continuation.zip'
CHECKSUM=ROOT.parent/'War_Room_OS_v57_Cusp_Falsification_Continuation.sha256.txt'
def sha256(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
def manifest_payload():
 ...
 return {'schema':'warroom.package_manifest.v57','release':'War_Room_OS_v57_Cusp_Falsification_Continuation','visual_application_version':'4.2','hardening_base':'5.2','research_accounting_base':'5.3','options_base':'5.6','cusp_continuation':'V73_V74_V75_COMPLETE_NEGATIVE_RESULT','files_digest_sha256':hashlib.sha256(can).hexdigest(),'files':rows,'claim_boundary':'Cusp catastrophe was tested under three frozen OOS/lockbox studies and rejected as an incremental crash/regime predictor. Descriptive density fit grants no direction, Crash Meter weight, or capital permission.'}
def safe(name):
...
def validate_zip(path):
 ...
 with zipfile.ZipFile(path) as z:
  ...
  with tempfile.TemporaryDirectory(prefix='v57_zip_') as td:
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='v57_build_') as td:
