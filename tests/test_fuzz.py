...
stages=set()
for _ in range(5000):
    row={'market':rng.choice(['US','IHSG','Crypto','FX','Commodity']),'research_action':rng.choice(['BUILD CANDIDATE','SELECTIVE ADD / WATCH','HOLD / NEEDS BETTER PRICE','SELL / AVOID','WATCH / NO FORCED TRADE']),'market_model_status':rng.choice(['RESEARCH READY / FUNDAMENTALS','RESEARCH READY / VALUE CAPTURE','PARTIAL / ECONOMICS','GATED']),'data_quality':rng.choice(['HIGH','MEDIUM','LOW']),'evidence_families':int(rng.integers(0,6)),'deterioration_families':int(rng.integers(0,5)),'price':float(abs(rng.normal(100,50))+0.1),'symbol':'BTC-USD'}
