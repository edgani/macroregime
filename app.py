...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with ihsg_tab:
        st.markdown("### 🇮🇩 IHSG — Indonesia Enhanced Narrative")
        st.caption("5-Layer Overlay: Sector Momentum · Commodity Proxy · Rupiah Regime · Foreign Flow · Domestic Macro")
        st.divider()
        # Ensure macro proxies are loaded (DXY, USDIDR, CL=F, HG=F, DBA)
        prices = _ensure_ihsg_macros(prices)
        ihsg_rows = []
        for ticker in list(IHSG_UNIVERSE.keys()):
            row = _build_ihsg_row_enhanced(ticker, prices, ar)
            if row:
                gamma_empty = {"ok": False}
                greek_empty = {"ok": False}
                row = _enrich_row_with_conclusions(row, gamma_empty, greek_empty, vix_now, prices.get(ticker))
        ...
        if not ihsg_rows:
            ...
            for r in ihsg_rows:
                df_data.append({
                    ...
                    "Grade": r["grade"],
                    "RR": r["rr"],
                    "Sector 1M": r["sector_momentum_1m"],
                    "Commodity": r["commodity_regime"],
                    "Rupiah": r["rupiah_regime"],
                    "Flow": r["foreign_flow"],
                    "Entry": r["entry"],
                    "T1": r["target_1"],
                    "T2": r["target_2"],
                })
            df_ihsg = pd.DataFrame(df_data)
            st.markdown("**📋 IHSG Setup Overview**")
            styled = df_ihsg.style
            for col in ["Direction"]:
                if col in df_ihsg.columns:
            ...
            for i, row in enumerate(longs):
                _render_ihsg_card(row, i)
            if not longs:
                st.info("No IHSG longs.")
            if shorts:
                st.divider()
