"""Build deterministic War Room OS V9.7 release ZIP and package manifest."""
...
EXCLUDE_SUFFIXES={".pyc",".pyo",".part",".tmp",".zip"}
EXCLUDE_FILES={"PACKAGE_MANIFEST.json",".env"}
...
def files(root:Path)->Iterable[Path]:
    for path in sorted(root.rglob('*'),key=lambda p:p.relative_to(root).as_posix().lower()):
        ...
        if rel.as_posix().startswith('runtime/v97_validation_fixture/'): continue
        yield path
...
def build(root:Path,output:Path)->dict:
    ...
    for path in files(root):
        rel=path.relative_to(root).as_posix(); entries.append({'path':rel,'bytes':path.stat().st_size,'sha256':sha(path)})
    manifest={'schema':'warroom.v97.package_manifest.v1','release':'War Room OS V9.7 Limited-Production Trading Control Plane','file_count_excluding_manifest':len(entries),'files':entries}
    ...
    return {'schema':'warroom.v97.release_build.v1','zip':str(output),'zip_sha256':sha(output),'zip_bytes':output.stat().st_size,'zip_members':len(members),'manifest_hash':manifest['manifest_hash'],'manifest_entries':len(entries)}
