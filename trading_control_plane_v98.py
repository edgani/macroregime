"""Fail-closed limited-production control plane for War Room OS V9.8.
...
POLICY_PATH = HERE / "V98_LIMITED_PRODUCTION_POLICY.json"
REGISTRY_PATH = HERE / "component_registry_v98.json"
QUOTE_PATH = HERE / "runtime" / "v98_trading" / "execution_quotes.json"
RUNTIME = HERE / "runtime" / "v98_trading"
...
def _policy(path: Path = POLICY_PATH) -> dict[str, Any]:
    policy = _load(path)
    if policy.get("schema") != "warroom.v98.limited_production_policy.v1":
    ...
    if _strict_bool(rules.get("auto_submit_enabled")) is not False:
        raise ValueError("V9.8 frozen policy must keep auto-submit disabled")
    return policy
...
def _verify_proof(decision: Mapping[str, Any]) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    try:
        ...
        return None, errors
    if proof.get("schema") not in {"warroom.v96.blind_proof_run.v1", "warroom.v97.blind_proof_run.v1", "warroom.v98.blind_proof_run.v1"}:
    ...
    try:
        ...
        if len(bound) != 1:
            errors.append("proof run is not uniquely bound in component_registry_v98.json")
    except Exception as exc:
        errors.append(f"V9.8 component registry invalid: {type(exc).__name__}: {exc}")
...
def evaluate(decision: Mapping[str, Any], account: Mapping[str, Any], quotes: Mapping[str, Any], *, now: dt.datetime | None = None, policy: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if decision.get("schema") != "warroom.v98.trade_decision.v1":
        errors.append("wrong decision schema")
    if account.get("schema") != "warroom.v98.account_state.v1":
    ...
    if direction == "FLAT":
        return {
            "schema": "warroom.v98.pretrade_evaluation.v1", "evaluated_at": _iso(now),
    ...
    return {
        "schema": "warroom.v98.pretrade_evaluation.v1",
...
def _approval_payload(evaluation: Mapping[str, Any], account: Mapping[str, Any], *, approved_by: str, now: dt.datetime) -> dict[str, Any]:
    return {
        "schema": "warroom.v98.human_approval.v1",
...
def verify_approval(approval: Mapping[str, Any], evaluation: Mapping[str, Any], account: Mapping[str, Any], *, secret: str) -> list[str]:
    errors: list[str] = []
    if approval.get("schema") != "warroom.v98.human_approval.v1":
...
def export_order(decision: Mapping[str, Any], account: Mapping[str, Any], evaluation: Mapping[str, Any], approval: Mapping[str, Any], *, secret: str, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    order = {
        "schema": "warroom.v98.broker_neutral_order.v1",
...
def engage_kill_switch(reason: str, *, actor: str) -> dict[str, Any]:
    RUNTIME.mkdir(parents=True, exist_ok=True)
    payload = {"schema": "warroom.v98.kill_switch.v1", "engaged": True, "engaged_at": _iso(_now()), "actor": actor, "reason": reason.strip() or "manual operator action"}
...
def release_kill_switch(*, actor: str, explicit_text: str) -> dict[str, Any]:
    if explicit_text != "RELEASE V98 KILL SWITCH":
    ...
    payload = {"schema": "warroom.v98.kill_switch_release.v1", "released_at": _iso(_now()), "actor": actor, "previous_hash": previous.get("record_hash"), "explicit_text": explicit_text}
    payload["record_hash"] = _hash({k: v for k, v in payload.items() if k != "record_hash"})
