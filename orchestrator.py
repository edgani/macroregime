...
def _build_alpha_center(prices, sq, mq, asset_ranges, health, alpha, bottlenecks, auto_discoveries, daily_signals, gamma_data=None, greeks_data=None):
    ...
    for b in bottlenecks.get("level_1", []):
        ...
        # Add conclusion fields for bottleneck
        gamma = gamma_data.get(t, {})
        greek = greeks_data.get(t, {})
        rr = asset_ranges.get(t, {})
        item = _enrich_item(item, t)
        item["worth_entering"] = "🚨 URGENT — " + b.get("direction", "HOLD")
        item["path_smoothness"] = "🔴 Rough — Volatility spike"
        item["time_estimate"] = "Immediate action"
        item["breakout_chance"] = "N/A — Risk management"
        item["entry_advice"] = b.get("setup", "Risk off positioning")
    ...
    for b in bottlenecks.get("level_2", []):
        ...
        item = _enrich_item(item, t)
        item["worth_entering"] = "⚠️ BUILDING — " + b.get("direction", "HOLD")
        item["path_smoothness"] = "🟡 Bumpy — Transition phase"
        item["time_estimate"] = "1-2 weeks"
        item["breakout_chance"] = "Medium — Watch for confirmation"
        item["entry_advice"] = b.get("setup", "Monitor closely")
        center_items.append(item)
    ...
    for b in bottlenecks.get("watch", []):
        ...
        item = _enrich_item(item, t)
        item["worth_entering"] = "👁️ WATCH — Wait for signal"
        item["path_smoothness"] = "⏳ Pending"
        item["time_estimate"] = "Monitor daily"
        item["breakout_chance"] = "Unknown — No position yet"
        item["entry_advice"] = b.get("setup", "Wait for clarity")
        center_items.append(item)
