...
assert 'active_scope=active.copy()' in ui
assert 'tmp=active_scope.copy()' in ui
assert 'alerts["event_id"].astype(str).isin(scoped_ids)' in ui
assert 'states["event_id"].astype(str).isin(scoped_ids)' in ui
assert 'outs["event_id"].astype(str).isin(scoped_ids)' in ui
