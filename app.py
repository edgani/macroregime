...
def _load_all():
    ...
    tickers = {
        ...
        "fx_longs": ["USDJPY=X","GLD","AAAU","YCS","USDCNH=X","USDCHF=X","USDSEK=X","USDNOK=X","IDR=X"],
        ...
        "indices": ["^VIX","^JKSE","QQQ","RSP","VUG","VTV","MTUM","USMV","HYG","LQD","XLB","XLRE","XLC","DX-Y.NYB"],
    }
...
def build_health(prices, f):
    ...
    vix=f.get("vix",20.0); hy=f.get("hy_oas",350.0)
...
with tabs[1]:
    ...
    show_hold = st.toggle("🔍 Show HOLD signals", False, key="show_hold_toggle")
    ...
    def render_fusion_card(ticker, name, ac, expected_sig):
        ...
        if ev is None:
            return None
        if not show_hold and ev["signal"] == "HOLD":
            return None
        trr_sig = ev["signal"]
    ...
    def rc_fusion(tlist, nmap, ac, expected_sig, per_row=2):
        ...
        for t in tlist:
            ...
            if card:
                cards.append(card)
        if not cards:
            st.caption("No actionable signals. Toggle 'Show HOLD' to see all watchlist.")
            return
...
with tabs[4]:
    ...
    dq_rows=[
        ...
        ("VIX Source", f"Regime snapshot ({vix:.1f})"),
        ...
        ("Treasury 2Y", f"{q.get('treasury_2y', '—')}%" ),
