...
- `tools/build_release.py` builds the repository release and matching SHA-256 sidecar through temporary files, preventing a stale sidecar when the ZIP is rebuilt.
- `tests/test_release_packaging.py` regression-checks checksum equality, required release content, duplicate paths, and runtime/cache exclusions.
- `tests/run_all.py` includes the packaging regression; the repository suite is now 27/27.
- Browser launch was retried against a live repository server through both Playwright and direct installed Chrome. Windows denied required IPC before page creation, so browser PASS and screenshots remain unclaimed.
