...
def build_shared_core(features: dict, raw: dict) -> dict:
    ...
    risk_range = RiskRangeEngine().run(raw.get('prices', {}), raw.get('price_frames', {}), features['market'], features['vol_credit'], positioning, features.get('derivatives', {}), shock)
    validation = ValidationEngine().run(raw.get('prices', {}), raw.get('price_frames', {}), risk_range.get('asset_ranges', {}))
    health = MarketHealthEngine().run(features['market'], validation)
        ...
        narrative_dict = {
            ...
            "early_stage_alerts": [
                {"name": c.name, "stage": c.stage, "conviction": c.conviction,
                 ...
                 "invalidators": c.invalidators}
                for c in narrative_output_obj.early_stage_alerts
            ],
    ...
    integrity = {
        ...
        'risk_range_model': risk_range.get('model', 'unknown'),
        'asset_range_coverage': int(risk_range.get('asset_range_coverage', 0) or 0),
        'ohlcv_coverage': float((raw.get('loader_meta', {}) or {}).get('prices', {}).get('ohlcv_share', 0.0) or 0.0),
