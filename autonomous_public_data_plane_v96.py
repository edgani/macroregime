"""War Room OS V9.6 resilient public/official data acquisition.
...
def collect_idx(root: Path, package_root: Path) -> dict[str, Any]:
    ...
    if not any("sha256" in item for item in items):
        handoff = {
            "schema": "warroom.v96.idx_browser_handoff.v1",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload = {
        "schema": "warroom.v96.autonomous_public_data_plane.v1",
        "generated_at": utc_now(),
    ...
    (snapshot_root / "v96_public_acquisition_manifest.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    return payload
...
def main() -> None:
    ...
    parser.add_argument("--output", default="runtime/v96_public_acquisition")
