"""V10.0 persistent current-context, decision and shadow worker."""
...
def _build(fast:bool)->dict:
 import current_context_v100 as CC
 ...
 desk.setdefault('runtime',{}).update({'core_collected_at':now_iso(),'core_profile':'V100_OPERATIONAL_CURRENT_ACTION','refresh_type':'FAST' if fast else 'FULL'})
 return desk
...
def run_once(*,fast:bool=True)->dict:
 ...
 try:
  ...
  if os.getenv('WARROOM_AUTO_SHADOW','0').lower() in {'1','true','yes'}:
   try:
    from shadow_runner_v100 import record
