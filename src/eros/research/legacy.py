...
def legacy_tilt_baseline(inflation_percentile: float, growth_velocity_percentile: float) -> dict[str, float]:
    raw = {"SPX": 0.25 - (inflation_percentile - 0.5), "TLT": 0.25 + (inflation_percentile - 0.5), "COMM": 0.25 + (growth_velocity_percentile - 0.5), "GLD": 0.25}
    floored = {asset: max(0.05, value) for asset, value in raw.items()}
    total = sum(floored.values())
    return {asset: value / total for asset, value in floored.items()}


def valuation_gap(filings: pd.DataFrame, decision_at: pd.Timestamp, market_cap: float, shares: float, current_price: float, historical_multiples: Sequence[float]) -> ValuationGapResult:
    ...
    return ValuationGapResult(ttm_fundamental=ttm, multiple=multiple, fair_value=fair, cheap_value=cheap, danger_value=danger, gap=fair / current_price - 1.0)
