...
Position size = f(conviction, asymmetry, max pain distance, gamma regime, boom-bust stage)
"""
...
def calculate_size(ticker: str, row: Dict, gamma: Dict, greek: Dict,
    ...
    stage_mult = 1.2 if boom_bust_stage in ("ACCELERATION", "SURVIVAL") else 0.7 if boom_bust_stage in ("MOMENT_OF_TRUTH", "TWILIGHT") else 0.4 if boom_bust_stage in ("TIP_POINT", "CRISIS") else 1.0
    ...
    if final_pct >= 10: mode = "🐷 PIG MODE"
    elif final_pct >= 5: mode = "🔥 SIZE UP"
    elif final_pct >= 2: mode = "✅ NORMAL"
    elif final_pct >= 1: mode = "⚠️ SMALL"
    else: mode = "❌ SKIP"

    return {
        "ticker": ticker, "size_pct": round(final_pct, 2), "size_dollar": round(portfolio_value * final_pct / 100, 0),
        "mode": mode, "base_pct": base_size_pct,
        "multipliers": {"grade": grade_mult, "rr": rr_mult, "entry": entry_mult, "gamma": gamma_mult, "stage": stage_mult, "reflexivity": ref_mult},
