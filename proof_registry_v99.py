"""V9.9 exact-market proof registry. A source/quote/ticker packet can never replace proof."""
from __future__ import annotations
import hashlib, json
from pathlib import Path
from typing import Any
from global_market_promotion_gate_v99 import evaluate_all
HERE=Path(__file__).resolve().parent
REGISTRY_PATH=HERE/'component_registry_v99.json'; POLICY_PATH=HERE/'NO_TECHNICAL_ANALYSIS_POLICY.json'
...
def load_registry()->dict[str,Any]:
    ...
    except Exception: raw={"version":"9.9","schema":"warroom.v99.component_registry.v1","components":{}}
    if raw.get('schema')!='warroom.v99.component_registry.v1' or not isinstance(raw.get('components'),dict):
        return {"version":"9.9","schema":"warroom.v99.component_registry.v1","components":{}}
    return raw
...
def component_status(name:str,row:dict[str,Any])->tuple[dict[str,Any],dict[str,Any]|None]:
    ...
    if path is None or not path.is_file(): reasons.append('V9.9 bound proof run not installed')
    ...
    if run is not None:
        if run.get('schema') not in {'warroom.v96.blind_proof_run.v1','warroom.v97.blind_proof_run.v1','warroom.v98.blind_proof_run.v1','warroom.v99.blind_proof_run.v1'}: reasons.append('wrong proof-run schema')
