"""Broker-neutral fill reconciliation and immutable execution ledger for War Room OS V9.7."""
...
def reconcile(order: Mapping[str, Any], fill: Mapping[str, Any]) -> dict[str, Any]:
    errors: list[str] = []
    if order.get("schema") != "warroom.v97.broker_neutral_order.v1":
        errors.append("wrong order schema")
    if fill.get("schema") != "warroom.v97.fill_receipt.v1":
    ...
    if errors:
        return {"schema": "warroom.v97.reconciliation.v1", "status": "REJECTED", "errors": sorted(set(errors)), "order_id": order.get("order_id")}
    ...
    record = {
        "schema": "warroom.v97.reconciliation.v1",
        "status": "RECONCILED",
...
def import_fill_csv(path: Path) -> dict[str, Any]:
    ...
    row = rows[0]
    return {
        "schema": "warroom.v97.fill_receipt.v1",
