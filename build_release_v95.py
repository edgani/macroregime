"""Build a deterministic War Room OS V9.5 release ZIP and package manifest."""
...
def files(root: Path) -> Iterable[Path]:
    for path in sorted(root.rglob("*"), key=lambda p: p.relative_to(root).as_posix().lower()):
        ...
        if not path.is_file() or any(part in EXCLUDE_DIRS for part in rel.parts):
            continue
        if path.name in EXCLUDE_FILES or path.suffix.lower() in EXCLUDE_SUFFIXES:
            continue
        if rel.as_posix().startswith("runtime/v95_public_acquisition_test/"):
            continue
        yield path
...
def build(root: Path, output: Path) -> dict:
    ...
    manifest = {
        "schema": "warroom.v95.package_manifest.v1",
    ...
    members = [(manifest_path, "PACKAGE_MANIFEST.json")] + [(root / row["path"], row["path"]) for row in entries]
    ...
    return {
        "schema": "warroom.v95.release_build.v1",
        "zip": str(output),
...
def main() -> None:
    p = argparse.ArgumentParser(); p.add_argument("--root", default="."); p.add_argument("--output", required=True)
    a = p.parse_args(); print(json.dumps(build(Path(a.root), Path(a.output)), indent=2))
