...
class OpportunityMemory:
    ...
    def record_state(self, event_id: str, lifecycle_state: str, *, observed_at_utc: Any = None,
        ...
        with self._connect() as cx:
            event = cx.execute("SELECT first_seen_time FROM opportunity_events WHERE event_id=?", (event_id,)).fetchone()
            if not event or pd.Timestamp(ts) < pd.Timestamp(event[0]):
                return False
            last = cx.execute("SELECT lifecycle_state,observed_at_utc FROM opportunity_state_history WHERE event_id=? ORDER BY observed_at_utc DESC,id DESC LIMIT 1", (event_id,)).fetchone()
            # State history is append-only in event time. Terminal episodes are immutable:
            # recurrence must go through create_event(), which allocates a new episode ID.
            if last and (pd.Timestamp(ts) < pd.Timestamp(last[1]) or (str(last[0]).upper() in {"INVALIDATED", "RESOLVED"} and state != str(last[0]).upper())):
                return False
