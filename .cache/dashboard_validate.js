...
function renderTop(){
 ...
 const chips=[
  ...
  `RADAR <b>${esc(String(obj(obj(D.current_developments).official_source_radar).changed_unreviewed||0))} REVIEW · ${esc(String(obj(D.current_developments).fresh_count||0))} FRESH</b>`,
...
function executionAction(marketId,marketBias,s){
 const sd=setupDirection(s), aligned=isSetupAligned(marketId,marketBias,s), valid=Boolean(s?.valid);
 const capitalBlocked=String(RK.global_permission||'CAPITAL_BLOCKED').toUpperCase()!=='PRODUCTION';
 if(marketId==='idx' && sd==='short')return 'REDUCE / AVOID';
 if(!aligned){
  if(sd==='long')return 'WATCH LONG';
  if(sd==='short')return 'WATCH SHORT';
  return 'NO TRADE';
 }
 if(sd==='long')return valid?(capitalBlocked?'TRIGGERED WATCH LONG':'BUILD LONG'):'WATCH LONG';
 if(sd==='short')return valid?(capitalBlocked?'TRIGGERED WATCH SHORT':'BUILD SHORT'):'WATCH SHORT';
 return 'NO TRADE';
}
function actionState(action){
 const x=String(action||'').toUpperCase();
 if(/TRIGGERED WATCH/.test(x))return 'watch';
 if(/BUILD LONG|BUY/.test(x))return 'constructive';
 if(/BUILD SHORT|REDUCE|AVOID/.test(x))return 'destructive';
 if(/WATCH|WAIT/.test(x))return 'watch';
 return 'NO_SIGNAL';
}
function marketPosture(marketId,bias){
 const bd=biasDirection(bias);
 if(marketId==='idx'&&bd==='short')return 'DEFENSIVE / WAIT';
 if(bd==='long')return 'LONG BIAS';
 if(bd==='short')return 'SHORT BIAS';
 return 'NO TRADE / TWO-SIDED WATCH';
}
function headroomPercentLabel(value){
 const x=String(value||'?').replace(/\s/g,'');
 if(x.includes('500x+'))return '+49,900%+';
 if(x.includes('50–500x')||x.includes('50-500x'))return '+4,900%–49,900%';
 if(x.includes('10–50x')||x.includes('10-50x'))return '+900%–4,900%';
 if(x.includes('3–10x')||x.includes('3-10x'))return '+200%–900%';
 if(x.includes('1.5–3x')||x.includes('1.5-3x'))return '+50%–200%';
 return 'SCENARIO HEADROOM';
}
function alphaTierRank(a){const t=Number(a?.tier||0);return Number.isFinite(t)?t:0;}
function researchActionLabel(value){
 const x=String(value||'WATCH').toUpperCase();
 if(/BUILD_LONG|BUILD LONG|BUY|LONG/.test(x))return 'WATCH LONG';
 if(/BUILD_SHORT|BUILD SHORT|SHORT|SELL/.test(x))return 'WATCH SHORT';
 return 'RESEARCH';
}
...
function firstCandidates(limit=8){
 ...
 Object.entries(obj(D.markets)).forEach(([marketId,m])=>{
  ...
  arr(m?.setups).forEach(s=>{
   if(!s?.tk||s.tk==='—')return;
   const fxMacroReady=marketId!=='fx'||String(m?.bias_state||'').toUpperCase()==='LIVE';
   const aligned=fxMacroReady&&isSetupAligned(marketId,bias,s);
...
function modelAlpha(){
 ...
 if(!rows.length)return emptyModel('ALPHA CENTER','Structural asymmetry universe has not loaded yet. Alpha is not replaced with a tactical price screener.');
 const instTickers=new Set([...arr(obj(D.institutional).events),...arr(LI.events)].map(x=>String(x.ticker||'').toUpperCase()).filter(Boolean));
 const fundTickers=new Set(arr(FLD.sec_fundamentals).map(x=>String(x.ticker||'').toUpperCase()).filter(Boolean));
 const enriched=rows.map(a=>{
  const ticker=String(a.tk||'').toUpperCase(), timing=obj(a.timing), market=a.market||'us';
  const inst=instTickers.has(ticker), fund=fundTickers.has(ticker), evidenceCount=Number(inst)+Number(fund);
  const action=timing&&Object.keys(timing).length?executionAction(market,obj(obj(D.markets)[market]).bias,timing):'NO TIMING';
  const mapped=!a.uncategorized, hidden=Boolean(a.is_hidden), active=/BUILD LONG|BUILD SHORT/.test(action);
  return {...a,ticker,inst,fund,evidenceCount,action,mapped,hidden,active,
   headroomPct:headroomPercentLabel(a.upside),
   proofState:evidenceCount===2?'MULTI-SOURCE':evidenceCount===1?'ONE SOURCE':'PROOF MISSING'};
 }).sort((a,b)=>(Number(b.mapped)-Number(a.mapped))||(Number(b.hidden)-Number(a.hidden))||(alphaTierRank(b)-alphaTierRank(a))||(b.evidenceCount-a.evidenceCount)||(num(b.asymmetry)-num(a.asymmetry)));
 const mappedCount=enriched.filter(x=>x.mapped).length, hiddenCount=enriched.filter(x=>x.hidden).length;
 const timingCount=enriched.filter(x=>x.action!=='NO TIMING').length, proofCount=enriched.filter(x=>x.evidenceCount>0).length;
 const universe=node('a_universe','EXTREME ALPHA UNIVERSE',`${meta.loaded_live_universe||0} live names + structural map`,`${enriched.length} RANKED`,'watch','observed',{raw:meta,w:220});
 const asym=node('a_asym','ASYMMETRY / HEADROOM',`${mappedCount} mapped · ${hiddenCount} hidden`,'STRUCTURAL SCREEN','watch','structural',{w:205});
 const proof=node('a_proof','INDEPENDENT PROOF',`${proofCount} have filed/institutional evidence`,proofCount?'PARTIAL':'MISSING','watch','observed',{w:205});
 const timing=node('a_timing','TIMING / EXECUTION',`${timingCount} have live price context`,'SEPARATE GATE','watch','inferred',{w:205});
 const extreme=enriched.filter(x=>x.mapped&&x.hidden&&alphaTierRank(x)>=4).sort((a,b)=>(num(b.asymmetry)||0)-(num(a.asymmetry)||0)).slice(0,2);
 const multiBag=enriched.filter(x=>x.mapped&&x.hidden&&alphaTierRank(x)===3).sort((a,b)=>(num(b.asymmetry)||0)-(num(a.asymmetry)||0)).slice(0,2);
 const timed=enriched.filter(x=>x.mapped&&x.price_loaded).sort((a,b)=>(num(b.asymmetry)||0)-(num(a.asymmetry)||0)).slice(0,2);
 const candidateRows=[...extreme,...multiBag,...timed,...enriched.filter(x=>x.mapped&&x.hidden).slice(0,5)].filter((x,i,a)=>a.findIndex(y=>y.ticker===x.ticker)===i).slice(0,5);
 const candidates=candidateRows.map((a,i)=>node('a_c_'+i,a.ticker,
  `STRUCTURAL CLASS · ${a.proofState}`,
  `${a.action} · EVIDENCE ${a.evidenceCount}/3`,
  a.active?actionState(a.action):'watch',a.mapped?'structural':'observed',{raw:a,market:a.market,w:194}));
 const g=layered([[universe],[asym,proof,timing],candidates]);
 g.edges.push(edge(universe.id,asym.id,'watch','structural','map scarcity',1.6,{relation:'structural_screen'}));
 g.edges.push(edge(asym.id,proof.id,'watch','inferred','prove value capture',1.6,{relation:'proof_gate'}));
 g.edges.push(edge(proof.id,timing.id,'watch','inferred','time separately',1.6,{relation:'timing_gate'}));
 candidates.forEach(n=>g.edges.push(edge(timing.id,n.id,n.state,'inferred','candidate state',1.3,{active:n.raw.active,relation:'candidate_timing'})));
 const lead=candidateRows[0]||enriched.find(x=>x.mapped&&x.hidden)||enriched[0];
 return {title:'ALPHA CENTER',sub:'Extreme-upside discovery: bottleneck, value capture, expectation gap and room-to-run. Tactical timing is only the last layer—not the definition of alpha.',canvas:'EXTREME ALPHA RADAR',badges:['structural','observed','inferred'],graph:g,
  rail:{...basicRail(`${lead.ticker} · STRUCTURAL RESEARCH CLASS`,
   `${lead.node||'Unclassified candidate'}. Numeric upside classes are withheld from the decision surface until a point-in-time value-capture model exists.`,
   `Universe ${enriched.length} · mapped ${mappedCount} · hidden ${hiddenCount} · proof ${proofCount} · live timing ${timingCount}.`,
   lead.action==='NO TIMING'?`Prove direct value capture, expectation gap and remaining return; then wait for a separate timing signal.`:`Prove direct value capture, expectation gap and remaining return; use ${lead.action} only as timing context.`,
   `Most high-headroom candidates fail, dilute, become crowded or never capture the bottleneck.`,
   `Kill on broken mechanism, substitution, dilution, missing proof or price invalidation.`,
   'Structural thesis: quarters/years · timing: setup-specific',Math.min(80,25+lead.evidenceCount*20+(lead.active?15:0)),'RESEARCH EXTREME ALPHA → THEN TIME ENTRY'),confidenceLabel:'EVIDENCE COMPLETENESS'},
  ledger:enriched.slice(0,40).map(a=>({time:obj(D.meta).generated,ticker:a.ticker,desc:`${a.domain||'unclassified'} · ${a.node||'mapping required'} · headroom ${a.upside} / base rate ${a.base_rate} · asymmetry A${Math.round(num(a.asymmetry)||0)} · ${a.proofState} · ${a.action}`,state:a.active?actionState(a.action):'RESEARCH',raw:a})),
  queue:enriched.slice(0,12).map(a=>({ticker:a.ticker,score:num(a.asymmetry)||0,why:`${a.proofState} · value capture and remaining return unassessed`,stage:a.action==='NO TIMING'?'STRUCTURAL RESEARCH':a.action}))};
}

function modelInstitutional(){
 const I=obj(D.institutional), statuses=arr(I.statuses), events=arr(I.events);
...
function modelMarket(id,title){
 const m=marketObj(id), allSetups=marketSetups(id), aligned=allSetups.filter(s=>isSetupAligned(id,m.bias,s)), b=breadthFor(id), tab=id==='idx'?'ihsg':id==='commodity'?'commodities':id==='crypto'?'crypto':'us_stocks';
 const derivCtx=marketDerivativeContext(id), liveRows=derivCtx.rows;
 const posture=marketPosture(id,m.bias);
 const stateNode=node('mbias','MARKET POSTURE',`${obj(m.funnel).universe||b.coverage||0} names loaded`,posture,posture,'inferred',{raw:m,w:205});
 ...
 const derivNode=node('mderiv',derivCtx.label,`${liveRows.length} normalized rows`,derivCtx.state,derivCtx.state,'observed',{raw:liveRows,w:190});
 const candidatePool=(aligned.length?aligned:allSetups).slice(0,5);
 const cards=candidatePool.map((s,i)=>{const action=executionAction(id,m.bias,s);const sub=isSetupAligned(id,m.bias,s)?`TRIG ${fmt(s.e)} · STOP ${fmt(s.s)}`:`WAIT ALIGNMENT · TRIG ${fmt(s.e)}`;return node('s_'+i,s.tk,sub,action,actionState(action),'observed',{raw:s,market:id,w:178});});
 const last=cards.length?cards:[node('sn','NO CURRENT SETUP','data loaded; no ticker passed setup construction','NO TRADE','NO_SIGNAL','observed',{w:220})];
 const g=layered([[stateNode,breadthNode,derivNode],last]);
 if(edgeAllowed(breadthNode,stateNode))g.edges.push(edge(breadthNode.id,stateNode.id,'watch','inferred','confirm posture',1.4,{relation:'breadth_confirmation'}));
 if(edgeAllowed(derivNode,stateNode))g.edges.push(edge(derivNode.id,stateNode.id,'watch','inferred','confirm posture',1.4,{relation:'derivative_confirmation'}));
 cards.forEach(n=>{if(isSetupAligned(id,m.bias,n.raw))g.edges.push(edge(stateNode.id,n.id,n.state,'inferred','action candidate',1.7,{active:/BUILD/.test(n.value),relation:'candidate_lineage'}));});
 const top=aligned[0]||allSetups[0], topAction=top?executionAction(id,m.bias,top):'NO TRADE';
 const watchText=candidatePool.slice(0,4).map(s=>`${s.tk} ${executionAction(id,m.bias,s)}`).join(' · ')||'No watchlist candidate.';
 const special=id==='idx'?'IHSG is cash long-only: bearish context means reduce/avoid or wait, never open a short.':id==='crypto'?'Crypto actions require venue/liquidity and derivatives confirmation.':'Action labels distinguish build, directional watch and no-trade states.';
 const devRows=arr(obj(obj(D.current_developments).by_market)[id]).filter(x=>x.freshness==='FRESH');
 const boardRows=allSetups.map(s=>{const a=executionAction(id,m.bias,s);return {instrument:s.tk,orientation:/SHORT/.test(a)?'SHORT':/LONG/.test(a)?'LONG':'NEUTRAL',action:a,state:actionState(a),why:s.why,coverage:num(s.conv)||0,total:100,trigger:s.e,stop:s.s,target:s.t,freshness:obj(D.meta).generated,event:'',raw:s};});
 return {title,sub:special,canvas:`${title} DECISION BOARD`,badges:['observed','inferred'],graph:g,
  board:{kind:id,rows:boardRows,developments:devRows,title:`${title} ACTION MATRIX`,note:id==='crypto'?'Includes dated structural changes: brokerage/onchain convergence, tokenized assets, stablecoins, prediction markets, agentic execution and regulation.':'Actions remain research states until prospective promotion.'},
  rail:{...basicRail(`${title}: ${posture}`,
   top?`${top.tk} is currently ${topAction}. Readiness is not probability.`:`No current setup.`,
   `ACTION NOW: ${aligned.length&&top?topAction:'NO TRADE'}. Watchlist: ${watchText}`,
   top?`Trigger ${fmt(top.e)} · stop ${fmt(top.s)} · reference target ${fmt(top.t)} · R/R ${fmt(top.rr)}.`:'Wait for a setup and market alignment.',
   `Alternative: posture can rotate while every ticker remains NO TRADE.`,
   top?(top.warn||`Invalidate at ${fmt(top.s)} or on posture reversal.`):'No setup means no trade-level invalidation.','Setup-specific',top&&isSetupAligned(id,m.bias,top)?Math.min(85,num(top.conv)||0):10,aligned.length&&top?topAction:'NO TRADE'),confidenceLabel:'SETUP COMPLETENESS'},
  ledger:[...allSetups.map(s=>{const a=executionAction(id,m.bias,s);return {time:obj(D.meta).generated,ticker:s.tk,desc:`${a} · ${s.why} · trigger ${fmt(s.e)} / stop ${fmt(s.s)} / reference ${fmt(s.t)} · ${isSetupAligned(id,m.bias,s)?'MARKET ALIGNED':'WAITING FOR ALIGNMENT'}`,state:actionState(a),raw:s};}),...marketDerivativeLedger(id),...coverageRows(tab)],
  queue:candidatePool.slice(0,8).map(s=>({ticker:s.tk,score:num(s.conv)||0,why:`trigger ${fmt(s.e)} · stop ${fmt(s.s)} · ${s.why}`,stage:executionAction(id,m.bias,s)}))};
}

function modelFx(){
 const id='fx',m=marketObj(id),b=breadthFor(id),fxs=obj(D.fx_pair_states),rows=arr(fxs.pairs),deriv=marketDerivativeContext(id);
 const loaded=rows.length||num(obj(m.funnel).universe)||num(b.coverage)||0;
 const triggered=rows.filter(x=>String(x.state||'').startsWith('TRIGGERED_WATCH'));
 const watches=rows.filter(x=>['WATCH_LONG','WATCH_SHORT'].includes(String(x.state||'')));
 const conflicts=rows.filter(x=>/CONFLICT|EVENT_RISK|PRICE_ONLY|NO_SIGNAL/.test(String(x.state||'')));
 const spot=node('fx_spot','PAIR-SPECIFIC FX',`${loaded} pair states`,rows.length?'LIVE':'NO_DATA',rows.length?'constructive':'NO_DATA','observed',{raw:fxs});
 ...
 const eventRows=arr(obj(obj(D.current_developments).by_market).fx).filter(x=>x.freshness==='FRESH');
 ...
 const gate=node('fx_gate','PAIR PERMISSION GATE','price-only never triggers; event risk downgrades',triggered.length?`${triggered.length} TRIGGERED WATCH`:'NO TRIGGERED WATCH',triggered.length?'watch':'NO_SIGNAL','inferred',{raw:fxs});
 ...
 return {title:'FX',sub:'No aggregate FX call. Every pair has an explicit LONG / SHORT / NEUTRAL / BLOCKED research state, driver coverage, event-risk gate, trigger and invalidation.',canvas:'FX PAIR DECISION BOARD',badges:['observed','inferred'],graph:g,
  board:{kind:'fx',rows:boardRows,developments:eventRows,title:'PAIR-SPECIFIC FX STATES',note:'Price-only states are visible but cannot become triggered watches. Capital remains blocked.'},
...
function modelValidation(){
 ...
 const statusOf=v=>String(v.grade||v.status||v.verdict||'UNKNOWN').toUpperCase();
 const validated=entries.filter(([,v])=>/VALIDATED|PRODUCTION/.test(statusOf(v))).length;
 const partial=entries.filter(([,v])=>/PARTIAL|MONITOR|RESEARCH/.test(statusOf(v))).length;
 const rejected=entries.filter(([,v])=>/REJECT|FAILED|BROKEN/.test(statusOf(v))).length;
 const inventory=node('val_inv','RESEARCH INVENTORY',`${entries.length} machine-readable grade cards`,'FROZEN SPECS','watch','observed',{w:205});
 const oos=node('val_oos','OOS / LOCKBOX',`${validated} currently validated`,'REQUIRED','watch','observed',{w:195});
 const correction=node('val_corr','SELECTION CONTROL',`${partial} partial · ${rejected} rejected`,'DSR / PBO / FDR','watch','observed',{w:205});
 const displayed=entries.slice().sort((a,b)=>{
   const rank=v=>/REJECT|FAILED|BROKEN/.test(statusOf(v))?0:/PARTIAL|MONITOR|RESEARCH/.test(statusOf(v))?1:2;
   return rank(a[1])-rank(b[1]);
 }).slice(0,8);
 const nodes=displayed.map(([k,v],i)=>{const status=statusOf(v);return node('v_'+i,k.replaceAll('_',' ').toUpperCase(),short(v.note||v.reason||'open grade card',31),status,status,'observed',{raw:v,w:184});});
 const center=node('prod','PRODUCTION GATE','baseline + costs + OOS + prospective','FAIL CLOSED','watch','observed',{w:220});
 const g=layered([[inventory,oos,correction],nodes.slice(0,4),nodes.slice(4,8),[center]]);
 nodes.forEach(n=>g.edges.push(edge(n.id,center.id,n.state,'inferred','grade gate',1.25,{relation:'validation_gate'})));
 ...
 rail:basicRail('PRODUCTION GATE',`${entries.length} graded components · ${validated} validated · ${partial} partial · ${rejected} rejected.`,`Only validated components may inform decisions inside their tested scope.`,`Monitor degradation, drift, calibration and untouched prospective outcomes.`,`Good in-sample results, attractive narratives and repeated tweaking are not production evidence.`,`Reject or downgrade on OOS failure, drift, broken lineage or failed prospective lockbox.`,'Continuous monitoring',Math.min(80,Math.round(validated/Math.max(1,entries.length)*100)),'OPEN GRADE CARD — NO AUTO PROMOTION'),ledger:[...entries.slice(0,30).map(([k,v])=>({time:obj(D.meta).generated,ticker:k,desc:v.note||v.reason||JSON.stringify(v),state:v.grade||v.status||'UNKNOWN'})),...arr(FLD.statuses).slice(0,30).map(x=>({time:x.fetched_at,ticker:x.provider,desc:`${x.dataset} · ${x.note}`,state:x.state,raw:x}))],queue:entries.slice(0,8).map(([k,v])=>({ticker:k,score:/VALID/i.test(v.grade||v.status||'')?90:/PARTIAL/i.test(v.grade||v.status||'')?55:15,why:v.note||v.reason||'',stage:v.grade||v.status||'UNKNOWN'}))};
}

function modelExecution(){
 const ids=[['us','US'],['idx','IHSG'],['crypto','CRYPTO'],['commodity','COMMOD'],['fx','FX']];
 const rows=[];
 ids.forEach(([id,label])=>{const m=marketObj(id);marketSetups(id).forEach(setup=>rows.push({market:id,label,setup,action:executionAction(id,m.bias,setup),aligned:isSetupAligned(id,m.bias,setup),score:num(setup.conv)||0}));});
 rows.sort((a,b)=>(Number(b.aligned)-Number(a.aligned))||(b.score-a.score));
 const marketNodes=ids.map(([id,label])=>{const m=marketObj(id),b=breadthFor(id);return node('ex_m_'+id,label,`${obj(m.funnel).universe||b.coverage||0} loaded · ${marketPosture(id,m.bias)}`,marketContextValue(id,m,b),marketContextState(id,m,b),'inferred',{market:id,raw:m,w:170});});
 const selected=rows.filter(x=>x.aligned).slice(0,5);
 const candidateNodes=(selected.length?selected:rows.slice(0,5)).map((r,i)=>node('ex_c_'+i,r.setup.tk,`TRIG ${fmt(r.setup.e)} · STOP ${fmt(r.setup.s)}`,r.action,actionState(r.action),'observed',{raw:r.setup,market:r.market,w:182}));
 const gate=node('ex_gate','CAPITAL PERMISSION','point-in-time OOS + lockbox + prospective','BLOCKED','watch','observed',{raw:RK,w:230});
 const g=layered([marketNodes,candidateNodes.length?candidateNodes:[node('ex_none','NO SETUP','No aligned execution context exists','NO TRADE','NO_SIGNAL')],[gate]]);
 candidateNodes.forEach(n=>{const parent=marketNodes.find(m=>m.market===n.market);if(parent&&edgeAllowed(parent,n))g.edges.push(edge(parent.id,n.id,n.state,'inferred','timing lineage',1.5,{active:false,relation:'execution_context'}));if(edgeAllowed(n,gate))g.edges.push(edge(n.id,gate.id,'watch','inferred','validation gate',1.2,{relation:'capital_gate'}));});
 const lead=selected[0]||rows[0];
 return {title:'EXECUTION & PORTFOLIO',sub:'Price context can create a triggered watch. Capital permission remains separate and fail-closed until prospective evidence exists.',canvas:'ACTION → INVALIDATION → CAPITAL GATE',badges:['observed','inferred'],graph:g,
  rail:{...basicRail('CAPITAL BLOCKED',lead?`${lead.setup.tk} · ${lead.action}. A triggered watch is not an autonomous order.`:'No aligned setup is available.',lead?`Trade invalidation ${fmt(lead.setup.s)} · trigger ${fmt(lead.setup.e)} · reference ${fmt(lead.setup.t)}.`:'No action now.',`Separate trade invalidation from thesis invalidation. Re-entry is allowed only after a new state is observed.`,`Correct thesis can still be a bad trade because of timing, leverage, liquidity or opportunity cost.`,`Kill on hard risk, broken mechanism, broken structure or superior alternative.`,'Setup-specific',15,'REVIEW → SIZE ZERO UNTIL PERMISSION'),confidenceLabel:'CAPITAL PERMISSION'},
  ledger:rows.slice(0,40).map(r=>({time:obj(D.meta).generated,ticker:r.setup.tk,desc:`${r.label} · ${r.action} · trigger ${fmt(r.setup.e)} / stop ${fmt(r.setup.s)} / reference ${fmt(r.setup.t)} · ${r.aligned?'ALIGNED':'WAITING ALIGNMENT'}`,state:actionState(r.action),raw:r})),
  queue:rows.slice(0,10).map(r=>({ticker:r.setup.tk,score:r.score,why:`${r.label} · trigger ${fmt(r.setup.e)} · stop ${fmt(r.setup.s)}`,stage:r.action}))};
}
...
function modelResearchProcess(){
 ...
 return {title:'RESEARCH LOOP',sub:'The operating system optimizes speed of falsification—not the number of backtests—and records uncertainty before outcomes are known.',canvas:'UNIVERSAL RESEARCH KERNEL',badges:['structural','observed','inferred'],graph:g,
  rail:basicRail('RESEARCH ONLY',`${entries.length} market-specific adapters share one discipline but not one formula.`,`Prediction, setup, expectation and contradictory evidence must be written before results are viewed.`,`Run the disposable version, tune the baseline, ablate complexity and attack the largest failure pile.`,`More experiments without a registry and untouched OOS increase p-hacking risk.`,`Invalidate on broken lineage, failed baseline lift, unstable regimes or failed prospective evidence.`,'Continuous',5,'FALSIFY CHEAPLY → DO NOT PROMOTE'),
  ledger:entries.flatMap(([id,k])=>[{time:obj(D.meta).generated,ticker:(k.label||id),desc:`Problem: ${k.decision_problem}`,state:'RESEARCH'},{time:obj(D.meta).generated,ticker:(k.label||id),desc:`Next falsifier: ${obj(k.validation).next_cheapest_falsifier||'UNASSESSED'}`,state:obj(k.validation).status||'RESEARCH_ONLY'}]),
  queue:missing.map(x=>({ticker:x.label,score:0,why:x.reason||'Prospective evidence missing',stage:x.state}))};
}

function modelDataHealth(){
 const health=obj(D.data_health), statuses=arr(health.sources), runtime=obj(D.runtime);
 const domains=[
...
function getModel(){switch(state.view){case'mc':return modelMission();case'macro':return modelMacro();case'ew':return modelEarly();case'alpha':return modelAlpha();case'inst':return modelInstitutional();case'deriv':return modelDerivatives();case'us':return modelMarket('us','US STOCKS');case'ihsg':return modelMarket('idx','IHSG');case'crypto':return modelMarket('crypto','CRYPTO');case'commod':return modelMarket('commodity','COMMODITIES');case'fx':return modelFx();case'flow':return modelFlow();case'sc':return modelSupply();case'co':return modelCompany();case'kg':return modelKnowledge();case'execution':return modelExecution();case'research':return modelResearchProcess();case'datahealth':return modelDataHealth();case'rc':return modelValidation();default:return modelMission();}}


function orientationClass(x){const v=String(x||'').toUpperCase();if(v==='LONG')return'long';if(v==='SHORT')return'short';if(v==='BLOCKED')return'blocked';return'neutral';}
function orientationFromText(x){const v=String(x||'').toUpperCase();if(/BLOCKED|NO_DATA|NO DATA|ACTION_REQUIRED|ERROR|FATAL/.test(v))return'BLOCKED';if(/SHORT|SELL|REDUCE|AVOID|DESTRUCTIVE|RISK OFF|RISK_OFF/.test(v))return'SHORT';if(/LONG|BUY|BUILD|CONSTRUCTIVE|LEAN_LONG|RISK ON|RISK_ON/.test(v))return'LONG';return'NEUTRAL';}
function normalizedBoard(model){
 ...
 let rows=[];
 if(queue.length){
  rows=queue.slice(0,18).map((q,i)=>{const action=q.stage||q.action||q.state||'RESEARCH';return {instrument:q.ticker||q.label||`ROW ${i+1}`,orientation:orientationFromText(action),action,state:action,why:q.why||rail.current||'',coverage:Math.max(0,Math.min(100,num(q.score)||num(rail.confidence)||0)),total:100,trigger:rail.next||'—',stop:rail.invalidation||'—',target:'—',freshness:obj(D.meta).generated,event:'',raw:q};});
 }
 if(!rows.length&&nodes.length){
  rows=nodes.slice(0,18).map((n,i)=>{const action=n.value||n.state||'OBSERVE';const evidence=String(n.evidence||'observed').toUpperCase();const cov=evidence==='OBSERVED'?85:evidence==='INFERRED'?55:evidence==='STRUCTURAL'?35:20;return {instrument:n.label||`NODE ${i+1}`,orientation:orientationFromText(`${action} ${n.state||''}`),action,state:n.state||action,why:n.sub||rail.current||'',coverage:cov,total:100,trigger:rail.next||'—',stop:rail.invalidation||'—',target:'—',freshness:obj(D.meta).generated,event:'',raw:n.raw||n};});
 }
...
function render(){
 ...
 document.querySelectorAll('[data-board-row]').forEach(el=>el.onclick=()=>{const key=el.dataset.boardRow;const row=arr(activeBoard.rows).find(x=>String(x.instrument)===String(key));if(row)openDetail(row.raw||row,key);});
...
function acceptSnapshot(next){
 ...
 D=next;LI=obj(D.live_intelligence);FLD=obj(D.full_live_data);RK=obj(D.research_kernel);sourceState=obj(D.data_health).overall||obj(D.meta).source||'NO_DATA';lastRevision=revision;lastContentHash=hash;lastPollOkAt=Date.now();
...
async function pollSnapshot(){
 ...
 try{
  ...
  try{const status=await fetchJsonBounded(STATUS_URL,2500);const sync=document.getElementById('dataSync');if(sync){const st=String(status.state||'UNKNOWN');sync.dataset.worker='1';const err=String(status.error||'');const html=`SYNC <b>${esc(st)} · R${lastRevision}${err?' · '+esc(short(err,55)):''}</b>`;if(sync.innerHTML!==html)sync.innerHTML=html;sync.classList.toggle('bad',/ERROR|FATAL/.test(st));}}catch(_){ }
