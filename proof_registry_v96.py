"""V9.6 exact-scope proof registry.

The dashboard consumes only hash-bound V9.6 proof-run outputs. Legacy receipt booleans and raw
editable metrics cannot activate a component. The global result is fail-closed unless all five exact
market proof runs independently pass the V9.6 cryptographic and recomputation firewall.
"""
...
REGISTRY_PATH = HERE / "component_registry_v96.json"
...
def load_registry() -> dict[str, Any]:
    try:
        ...
        raw = {"version": "9.6", "schema": "warroom.v96.component_registry.v1", "components": {}}
    if raw.get("schema") != "warroom.v96.component_registry.v1" or not isinstance(raw.get("components"), dict):
        return {"version": "9.6", "schema": "warroom.v96.component_registry.v1", "components": {}}
    return raw
...
def component_status(component: str, row: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    if path is None or not path.is_file():
        reasons.append("V9.6 proof run not installed")
    ...
    out.update({
        ...
        "state": "HUMAN_APPROVED_LIMITED_PRODUCTION" if valid else "AWAITING_BOUND_V96_PROOF",
    })
...
def attach_proof_registry(desk: dict) -> dict:
    ...
    try:
        ...
        desk["no_technical_analysis_policy"] = {"capital_default": "BLOCKED", "effective_version": "9.6"}
    desk["proof_status"] = {
        ...
        "operational_permission": "RESEARCH_AND_SHADOW_READY",
        "software_is_not_alpha": True,
        "evidence_production_runtime_ready": True,
        "proof_firewall_version": "9.6",
    }
    return desk
