...
"""
    ...
    if not active.empty:
        ...
        if selected_markets is not None:
            ...
            left=left[left["market"].astype(str).isin(allowed)]
        query=str(search_query or st.session_state.get("mq_global_search_v322","")).strip().lower()
        if query:
            match=pd.Series(False,index=left.index)
            for col in ["symbol","asset","theme","sector","industry"]:
                if col in left:
                    match=match | left[col].astype(str).str.lower().str.contains(query,regex=False,na=False)
            left=left[match]
