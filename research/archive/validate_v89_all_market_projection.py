...
def main() -> int:
    ...
    check("CLI identifies V9.0", "WAR ROOM OS V9.0" in run_text and "War Room OS V9.0" in run_text)
    check("Windows operator scripts present", all((HERE / name).exists() for name in ("SETUP_V88.bat", "CHECK_V88.bat", "RUN_V88.bat")))
    readme = (HERE / "README.md").read_text(encoding="utf-8")
    check("README preserves capital-blocked claim limit", "blocked" in readme.lower() and "does not claim a trading edge" in readme.lower() and "no technical indicator" in readme.lower())
    check("dashboard identifies V9.0", "V9.0" in html and "PRICE PROJECTION" in html)
    ...
    report = {
        "schema": "warroom.v88.final_validation.v1", "passed": passed, "total": len(TESTS),
    ...
    (HERE / "V88_FINAL_VALIDATION.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
