"""V9.7 exact-scope proof registry with order-time recomputation boundary."""
from __future__ import annotations
import hashlib, json
from pathlib import Path
from typing import Any
from global_market_promotion_gate_v97 import evaluate_all
...
def load_registry() -> dict[str, Any]:
    ...
    except Exception: raw={"version":"9.7","schema":"warroom.v97.component_registry.v1","components":{}}
    if raw.get("schema")!="warroom.v97.component_registry.v1" or not isinstance(raw.get("components"),dict):
        return {"version":"9.7","schema":"warroom.v97.component_registry.v1","components":{}}
    return raw
...
def component_status(name: str, row: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    if path is None or not path.is_file(): reasons.append("V9.7 bound proof run not installed")
    ...
    if run is not None:
        if run.get("schema") not in {"warroom.v96.blind_proof_run.v1","warroom.v97.blind_proof_run.v1"}: reasons.append("wrong proof-run schema")
    ...
    out.update({
        "proof_run_valid":valid,"proof_run_hash":expected if valid else None,"proof_run_reasons":sorted(set(reasons)),
        "decision_active":valid,"capital_permission":"LIMITED_PRODUCTION_ELIGIBLE" if valid else "BLOCKED",
        "live_weight":1.0 if valid else 0.0,"state":"AWAITING_HUMAN_ORDER_APPROVAL" if valid else "AWAITING_BOUND_V97_PROOF",
    })
...
def attach_proof_registry(desk: dict) -> dict:
    ...
    except Exception: desk["no_technical_analysis_policy"]={"capital_default":"BLOCKED","effective_version":"9.7"}
    desk["proof_status"]={
        "final_trading_system":bool(global_result["global_trading_ready"]),"all_market_trading_ready":bool(global_result["global_trading_ready"]),
        "predictive_components_promoted":len(authorized),"decision_active_predictive_components":len(authorized),"capital_authorized_components":authorized,
        "missing_market_components":[m for m in ("us","idx","commodity","fx","crypto") if m not in runs],
        "capital_permission":global_result["capital_permission"],"operational_permission":"LIMITED_PRODUCTION_CONTROL_PLANE_READY",
        "software_is_not_alpha":True,"order_export_requires_human_approval":True,"auto_submit":False,"proof_firewall_version":"9.7",
    }
    return desk
