...
def export(output:Path|None=None)->dict:
 ...
 stamp=dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%SZ');out=output or HERE/'runtime'/'v100_orders'/f'V100_REVIEW_{stamp}.csv';out.parent.mkdir(parents=True,exist_ok=True)
 ...
 result={'schema':'warroom.v100.order_review_export.v1','created_at':stamp,'rows':len(rows),'path':str(out.relative_to(HERE)),'submission':'DISABLED','claim_limit':'Manual review worksheet only; no broker transmission.'}
