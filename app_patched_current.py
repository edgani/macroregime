...
# ── Refactor helpers (baseline-preserving) ───────────────────────────────────
def _page_to_market_key(page_label:str)->str:
    return {
        'US Stocks':'US',
        'IHSG':'IHSG',
        'Forex':'FX',
        ...
        'Crypto':'Crypto',
    }.get(page_label, page_label)


def _market_universe(market:str)->List[str]:
    if market=='US': return list(US_TICKERS)
    if market=='IHSG': return list(IHSG_TICKERS)
    if market=='FX': return list(FX_TICKERS)
    if market=='Commodities': return list(COMM_TICKERS)
    if market=='Crypto': return list(CRYPTO_TICKERS)
    return []


def _simple_price_window(px:float, bias:str)->Tuple[str,str,str]:
    if not math.isfinite(px) or px<=0:
        return '—','—','—'
    wiggle=max(px*0.025, 0.01)
    if bias in {'LONG','Buy'}:
        entry=f"{px-wiggle:.2f} – {px:.2f}"
        invalid=f"<{px-1.8*wiggle:.2f}"
        target=f"{px+2.5*wiggle:.2f}"
    else:
        entry=f"{px:.2f} – {px+wiggle:.2f}"
        invalid=f">{px+1.8*wiggle:.2f}"
        target=f"{px-2.5*wiggle:.2f}"
    return entry, invalid, target


def _fallback_market_rows(snap:Dict, market:str, n:int=5)->Dict[str,List[Dict]]:
    prices=snap.get('prices',{}) or {}
    tickers=_market_universe(market)
    rows=[]
    for tk in tickers:
        s=_s(prices.get(tk,pd.Series(dtype=float)))
        if len(s)<20:
        ...
        if not math.isfinite(r1):
            continue
        px=last(s)
        rows.append({'tk':tk,'r1':r1,'px':px})
    rows_long=sorted(rows,key=lambda x:x['r1'],reverse=True)
    rows_short=sorted(rows,key=lambda x:x['r1'])

    def make_row(item,bias:str,bucket:str,why:str,score_scale:float=1.0):
        entry, invalid, target = _simple_price_window(float(item['px']), 'Buy' if bias=='Buy' else bias)
        return {
            'Ticker': disp(item['tk']) if market!='IHSG' else item['tk'].replace('.JK','')+' (JK)',
            'Bias': bias,
            'Bucket': bucket,
            'Entry Zone': entry,
            'Invalidation': invalid,
            'Target': target,
            'Score': f"{max(0.0,min(abs(item['r1'])*100*score_scale,99.0)):.0f}%",
            'Why Now': why,
            'Trigger': f"Break/hold around {entry}",
            'Why Not Yet': 'Need cleaner confirmation',
            'Trigger Distance': f"{abs(item['r1'])*10:.1f}",
            'Problem': 'Weak momentum / route mismatch',
            'Why Avoid': 'Not among best expressions right now',
            'Better Alternative': 'See top rows above',
            '_raw_ticker': item['tk'],
        }

    out={'now_long':[],'now_short':[],'front_long':[],'front_short':[],'avoid':[],'defensive':[]}
    if market=='IHSG':
        out['now_long']=[make_row(x,'Buy','Buy Now','Strong relative momentum in current board') for x in rows_long[:n]]
        out['front_long']=[make_row(x,'Buy','Front-Run Buy','Improving but not yet fully confirmed',0.8) for x in rows_long[n:n*2]]
        out['avoid']=[make_row(x,'Avoid','Avoid / Reduce','Weak relative momentum vs peers',0.7) for x in rows_short[:n]]
        def_names=['ICBP.JK','KLBF.JK','TLKM.JK','BBCA.JK']
        defensive=[]
        for tk in def_names:
            s=_s(prices.get(tk,pd.Series(dtype=float)))
            if len(s)>=20:
                defensive.append(make_row({'tk':tk,'r1':ret_n(s,21),'px':last(s)},'Defensive','Defensive Shelter','Higher-quality / steadier profile in stress',0.6))
        out['defensive']=defensive[:n]
    else:
        out['now_long']=[make_row(x,'Long','Best Longs Now','Strongest 1M momentum and trend support') for x in rows_long[:n]]
        out['now_short']=[make_row(x,'Short','Best Shorts Now','Weakest 1M momentum / fade candidate') for x in rows_short[:n]]
        out['front_long']=[make_row(x,'Long','Front-Run Longs','Improving but not yet top-ranked',0.8) for x in rows_long[n:n*2]]
        out['front_short']=[make_row(x,'Short','Front-Run Shorts','Weak but not fully broken yet',0.8) for x in rows_short[n:n*2]]
        out['avoid']=[make_row(x,'Avoid','Avoid / Reduce','Not clean enough versus better alternatives',0.6) for x in rows_short[:n]]
    return out


def _build_market_action_bundle(snap:Dict, market:str)->Dict:
    opps=snap.get('opportunities',[]) or []
    prices=snap.get('prices',{}) or {}
    market_rows=[r for r in opps if str(r.get('Market',''))==market]
    now_long=[]; now_short=[]; front_long=[]; front_short=[]
    for r in market_rows:
        bias=str(r.get('Bias',''))
        row={
            'Ticker': r.get('Ticker','—'),
            'Bias': bias,
            'Bucket': r.get('Rally Fit', r.get('Path', r.get('Setup',''))),
            'Entry Zone': r.get('Entry Zone','—'),
            'Invalidation': r.get('Invalidation','—'),
            'Target': r.get('Target','—'),
            'Score': r.get('EV','—'),
            'Why Now': r.get('Why Now','—'),
            'Trigger': r.get('Entry Zone','—'),
            'Why Not Yet': r.get('Why Not Yet','—'),
            'Trigger Distance': r.get('Conf','—'),
            'Problem': r.get('Why Not Yet','—'),
            'Why Avoid': r.get('Why Not Yet','—'),
            'Better Alternative': 'See best row in same market',
            '_raw_ticker': r.get('_raw_ticker', r.get('Ticker','')),
        }
        if bias.startswith('WATCH-LONG'):
            front_long.append(row)
        elif bias.startswith('WATCH-SHORT'):
            front_short.append(row)
        elif 'LONG' in bias:
            now_long.append(row)
        elif 'SHORT' in bias:
            now_short.append(row)

    fallback=_fallback_market_rows(snap, market, n=5)
    if market=='IHSG':
        buy_now=(now_long or fallback['now_long'])[:5]
        front_buy=(front_long or fallback['front_long'])[:5]
        avoid=(fallback['avoid'])[:5]
        defensive=(fallback['defensive'])[:5]
        return {
            'buy_now': buy_now,
            'front_run_buy': front_buy,
            'avoid_reduce': avoid,
            'defensive_shelter': defensive,
        }

    strong_weak=snap.get('strong_weak_all',{}) or {}
    avoid_rows=[]
    key={'US':'us','FX':'fx','Commodities':'commodities','Crypto':'crypto'}.get(market,'')
    if market=='US':
        sw=build_strong_weak(prices, snap.get('q',{}).get('quad','Q3'), limit=5)
        for r in sw.get('weak',[])[:5]:
            tk=r.get('Ticker')
            s=_s(prices.get(tk,pd.Series(dtype=float)))
            if len(s)<5: continue
            px=last(s); entry,invalid,target=_simple_price_window(px,'Short')
            avoid_rows.append({'Ticker':tk,'Bias':'Avoid','Bucket':'Weak / Avoid','Entry Zone':entry,'Invalidation':invalid,'Target':target,'Score':'—','Why Now':'Weak relative momentum','Trigger':entry,'Why Not Yet':'—','Trigger Distance':'—','Problem':'Weak relative momentum','Why Avoid':'Lower-quality setup vs stronger names','Better Alternative':'See top longs / shorts','_raw_ticker':tk})
    elif key and strong_weak.get(key):
        for r in strong_weak[key].get('weak',[])[:5]:
            tk=r.get('tk')
            s=_s(prices.get(tk,pd.Series(dtype=float)))
            if len(s)<5: continue
            px=last(s); entry,invalid,target=_simple_price_window(px,'Short')
            avoid_rows.append({'Ticker':r.get('name',disp(tk)),'Bias':'Avoid','Bucket':'Weak / Avoid','Entry Zone':entry,'Invalidation':invalid,'Target':target,'Score':'—','Why Now':'Weak relative momentum','Trigger':entry,'Why Not Yet':'—','Trigger Distance':'—','Problem':'Weak relative momentum','Why Avoid':'Lower-quality setup vs stronger names','Better Alternative':'See best rows above','_raw_ticker':tk})

    return {
        'now_long': (now_long or fallback['now_long'])[:5],
        'now_short': (now_short or fallback['now_short'])[:5],
        'front_run_long': (front_long or fallback['front_long'])[:5],
        'front_run_short': (front_short or fallback['front_short'])[:5],
        'avoid_reduce': (avoid_rows or fallback['avoid'])[:5],
    }


def _render_rows_table(rows:List[Dict], title:str, cols:List[str])->None:
    sh(title)
    if not rows:
        st.info('Belum ada kandidat yang qualified.')
        return
    df=pd.DataFrame(rows)
    use=[c for c in cols if c in df.columns]
    st.dataframe(df[use], use_container_width=True, hide_index=True, height=min(max(180, len(df)*35+46), 360))


def render_ticker_attack_matrix(snap:Dict)->None:
    sh('🎯 TICKER ATTACK MATRIX')
    matrix=[]
    for market in ['US','IHSG','FX','Commodities','Crypto']:
        bundle=_build_market_action_bundle(snap, market)
        if market=='IHSG':
            matrix.append({
                'Market': market,
                'Best Longs / Buys Now': ', '.join([r['Ticker'] for r in bundle['buy_now'][:3]]) or '—',
                'Best Shorts Now': '—',
                'Front-Run Longs / Buys': ', '.join([r['Ticker'] for r in bundle['front_run_buy'][:3]]) or '—',
                'Front-Run Shorts': '—',
            })
        else:
            matrix.append({
                'Market': market,
                'Best Longs / Buys Now': ', '.join([r['Ticker'] for r in bundle['now_long'][:3]]) or '—',
                'Best Shorts Now': ', '.join([r['Ticker'] for r in bundle['now_short'][:3]]) or '—',
                'Front-Run Longs / Buys': ', '.join([r['Ticker'] for r in bundle['front_run_long'][:3]]) or '—',
                'Front-Run Shorts': ', '.join([r['Ticker'] for r in bundle['front_run_short'][:3]]) or '—',
            })
...
def page_dashboard_control_tower(snap:Dict)->None:
    opps=snap.get('opportunities',[]) or []
    longs=[r for r in opps if 'LONG' in str(r.get('Bias','')) and not str(r.get('Bias','')).startswith('WATCH')]
    shorts=[r for r in opps if 'SHORT' in str(r.get('Bias','')) and not str(r.get('Bias','')).startswith('WATCH')]
    watchs=[r for r in opps if str(r.get('Bias','')).startswith('WATCH')]
    most_hated=snap.get('most_hated_rally',{}) or {}
    clear_count=int(most_hated.get('clear_count',0) or 0)
    c1,c2,c3,c4=st.columns(4)
    with c1: mc('Rally State', most_hated.get('stage','monitor'), f'{clear_count}/4 checklist', 'good' if clear_count>=3 else ('warn' if clear_count>=2 else 'bad'))
    with c2: mc('Action', most_hated.get('action','Selective')[:24], most_hated.get('posture',''), 'good' if clear_count>=3 else ('warn' if clear_count>=2 else 'bad'))
    with c3:
        cr=snap.get('crash',{}) or {}
        mc('Exec Mode', cr.get('exec_mode','—'), f"score {float(cr.get('exec_score',0) or 0):.0%}", 'good' if float(cr.get('exec_score',0) or 0)>=0.60 else 'warn')
    with c4:
        q=snap.get('q',{}) or {}
        mc('Board', f"{len(longs)}L / {len(shorts)}S / {len(watchs)}W", f"{q.get('quad','?')} · {float(q.get('confidence',0) or 0):.0%} conf", 'good' if len(longs)>=len(shorts) else 'warn')

    if most_hated:
        render_most_hated_rally_monitor(most_hated, compact=True)
    render_top_drivers_now(snap.get('top_drivers',[]) or [], title='🧠 TOP DRIVERS NOW — apa yang gerakin tape sekarang')

    route=snap.get('route',{}) or {}
    rc1,rc2,rc3=st.columns(3)
    with rc1: mc('Current Path', route.get('primary_meta',{}).get('label', route.get('primary','—')), route.get('primary_meta',{}).get('desc',''))
    with rc2: mc('Next Path', route.get('alt_meta',{}).get('label', route.get('alt','—')), route.get('alt_meta',{}).get('desc',''))
    with rc3: mc('Risk State', (snap.get('crash',{}) or {}).get('state','—'), (snap.get('crash',{}) or {}).get('exec_mode','—'))

    render_ticker_attack_matrix(snap)

    t1,t2,t3=st.tabs(['🎯 Scenarios & What-If','🌐 Cross-Asset','📡 Health / Breadth'])
    with t1:
        page_playbook(snap)
    with t2:
        page_radar(snap)
...
def page_market_action(snap:Dict, market:str)->None:
    bundle=_build_market_action_bundle(snap, market)
    if market=='IHSG':
        _render_rows_table(bundle['buy_now'], '🟢 BUY NOW', ['Ticker','Bucket','Entry Zone','Invalidation','Target','Score','Why Now'])
        _render_rows_table(bundle['front_run_buy'], '🟡 FRONT-RUN BUY', ['Ticker','Bucket','Trigger','Why Not Yet','Trigger Distance','Score'])
        page_signal_strength(snap, forced_market='IHSG', key_suffix='ihsg_refactor', compact=True, show_header=True)
        _render_rows_table(bundle['avoid_reduce'], '🔴 AVOID / REDUCE', ['Ticker','Bucket','Problem','Why Avoid','Better Alternative'])
        _render_rows_table(bundle['defensive_shelter'], '🛡️ DEFENSIVE SHELTER / CASH', ['Ticker','Bucket','Why Now'])
        with st.expander('Legacy detail', expanded=False):
            page_ihsg(snap)
        return

    forced={'US':'US','FX':'FX','Commodities':'Commodities','Crypto':'Crypto'}[market]
    _render_rows_table(bundle['now_long'], '🟢 BEST LONGS NOW', ['Ticker','Bucket','Entry Zone','Invalidation','Target','Score','Why Now'])
    _render_rows_table(bundle['now_short'], '🔴 BEST SHORTS NOW', ['Ticker','Bucket','Entry Zone','Invalidation','Target','Score','Why Now'])
    _render_rows_table(bundle['front_run_long'], '🟡 FRONT-RUN LONGS', ['Ticker','Bucket','Trigger','Why Not Yet','Trigger Distance','Score'])
    _render_rows_table(bundle['front_run_short'], '🟠 FRONT-RUN SHORTS', ['Ticker','Bucket','Trigger','Why Not Yet','Trigger Distance','Score'])
    page_signal_strength(snap, forced_market=forced, key_suffix=f'{market.lower()}_refactor', compact=True, show_header=True)

    mi=snap.get('macro_impact',{}) or {}
    chk=snap.get('asset_checklists',{}) or {}
    sw=snap.get('strong_weak_all',{}) or {}
    sh('📊 MARKET STATE')
    mk_map={'US':'us','FX':'fx','Commodities':'commodities','Crypto':'crypto'}
    mk=mk_map.get(market,'')
    c1,c2=st.columns(2)
    with c1:
        bd=mi.get(mk,{})
        if bd:
            mc('Sekarang', bd.get('quad','—'), bd.get('now','—'))
            mc('Best Expression', '→', bd.get('best_expression','—'))
    with c2:
        if sw.get(mk):
            strong=', '.join([r.get('name', r.get('Ticker','')) for r in sw[mk].get('strong',[])[:4]])
            weak=', '.join([r.get('name', r.get('Ticker','')) for r in sw[mk].get('weak',[])[:4]])
            mc('Strong', strong or '—')
            mc('Weak', weak or '—')
    if chk.get(mk):
        render_checklist(chk[mk], f'{market.upper()} CHECKLIST')

    _render_rows_table(bundle['avoid_reduce'], '⚠️ AVOID / REDUCE', ['Ticker','Bucket','Problem','Why Avoid','Better Alternative'])


def main():
    st.markdown('<div style="display:flex;align-items:center;margin-bottom:4px"><span style="font-family:Syne,sans-serif;font-size:20px;font-weight:800;letter-spacing:-.03em">🧭 MacroRegime Pro</span><span style="font-size:10px;opacity:.3;margin-left:8px;font-family:DM Mono,monospace">v10.1 · Refactor patch on legacy baseline</span></div>',unsafe_allow_html=True)
    with st.sidebar:
    ...
    snap=load_all()
    most_hated=snap.get("most_hated_rally",{})
    q=snap["q"]; cr=snap["crash"]; quad=q["quad"]; meta=QUAD_META.get(quad,{})
    ...
    best_long=next((o for o in opps_all if "LONG" in o.get("Bias","") and not str(o.get('Bias','')).startswith('WATCH')),None)
    best_short=next((o for o in opps_all if "SHORT" in o.get("Bias","") and not str(o.get('Bias','')).startswith('WATCH')),None)
    ...
    status_html = (
        f'<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;padding:8px 12px;'
        f'border-radius:8px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);'
        f'margin-bottom:10px;font-size:11px">'
        f'<span>{qb(quad)}{div_badge} <strong>{meta.get("label","")}</strong></span>'
        f'<span style="opacity:.2">|</span>'
        f'<span style="color:{route_color};font-weight:700">{route_emoji} {route_label}</span>'
        f'<span style="opacity:.2">|</span>'
        f'<span>Conf: <strong>{q.get("confidence",0):.0%} ({q.get("conf_band","")})</strong></span>'
        f'<span style="opacity:.2">|</span>'
        f'<span>Growth: <strong>{ga}</strong> | Inflasi: <strong>{ia}</strong></span>'
        f'<span style="opacity:.2">|</span>'
        f'<span>▲ Best Long: <strong>{best_long_tk}</strong></span>'
        f'<span style="opacity:.2">|</span>'
        f'<span>▼ Best Short: <strong>{best_short_tk}</strong></span>'
        f'<span style="opacity:.2">|</span>'
        f'<span>Risk: <strong>{cr.get("state","—")}</strong> | Exec: <strong>{cr.get("exec_mode","—")}</strong></span>'
        + (f'<span style="opacity:.2">|</span><span>Rally Trigger: <strong>{most_hated.get("clear_count","—")}/4</strong></span>' if most_hated else '')
        + (f'<span style="opacity:.2">|</span><span style="opacity:.6">{news_label}</span>' if news_label else '')
        + f'<span style="opacity:.25;margin-left:auto">{snap.get("ts","—")}</span></div>'
    )
    ...
    if page=='Dashboard':
        ...
        page_market_action(snap,'US')
    elif page=='IHSG':
        page_market_action(snap,'IHSG')
    elif page=='Forex':
        page_market_action(snap,'FX')
    elif page=='Commodities':
        page_market_action(snap,'Commodities')
    elif page=='Crypto':
        page_market_action(snap,'Crypto')
    elif page=='Risk':
        page_risk(snap)
    else:
        page_diag(snap)
