"""Strict deterministic package manifest verifier for War Room OS v5.5."""
from __future__ import annotations
import hashlib,json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
MANIFEST=ROOT/'PACKAGE_MANIFEST_V55.json'
IGNORE_DIRS={'.git','.venv','__pycache__','.cache','.pytest_cache','audit_logs','runtime','audit_state_v55'}
IGNORE_EXACT={
 'PACKAGE_MANIFEST_V55.json','PACKAGE_MANIFEST_V53.json','V55_USER_VALIDATION_REPORT.json',
...
def verify(root:Path=ROOT,manifest_path:Path|None=None)->dict:
 ...
 if raw.get('schema')!='warroom.package_manifest.v55':res['errors'].append('manifest schema mismatch')
