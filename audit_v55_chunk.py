"""Resumable fresh-copy audit chunks for v5.5."""
from __future__ import annotations
import hashlib,json,os,shutil,subprocess,sys,tempfile
from pathlib import Path
ROOT=Path(__file__).resolve().parent
STATE=ROOT/'audit_state_v55';STATE.mkdir(exist_ok=True)
IGNORE_DIRS={'__pycache__','.git','.cache','.pytest_cache','runtime','audit_logs','.venv','audit_state_v55'}
GEN_PREFIX=('V42_','V52_','V53_','V55_','V70_','V71_','PACKAGE_MANIFEST_')
...
def manifest(root):
 ...
 for p in sorted(root.rglob('*')):
  ...
  if p.name.startswith(GEN_PREFIX) and not s.startswith(('research_v55/V69_','research_v55/V70_','research_v55/V71_')):continue
  if p.suffix.lower()=='.json' and not s.startswith(('research_v55/V69_','research_v55/V70_','research_v55/V71_')):continue
  if p.suffix.lower() in {'.py','.html','.md','.txt','.csv','.parquet','.yaml','.yml','.toml','.bat'} or s.startswith(('research_v55/V69_','research_v55/V70_','research_v55/V71_')):out[s]=h(p)
 return out
def copy(dst):
...
SPECS={
 ...
 'synthetic':([sys.executable,'run.py','--synthetic','--markets','us,idx,crypto,commodity,fx','--out','runtime/v55_desk.json','--html','runtime/v55_dashboard.html'],480,set()),
...
def main(names):
 with tempfile.TemporaryDirectory(prefix='v55_chunk_') as td:
