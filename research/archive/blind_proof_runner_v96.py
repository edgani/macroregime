"""Cryptographically bound exact-market proof runner for War Room OS V9.6.

The V9.1 runner could evaluate supplied CSVs while separately accepting manually asserted receipt
metrics and placeholder-looking artifact hashes. V9.6 binds every supplied artifact to the forecast
...
def run(*, predictor_manifest: Path, outcome_manifest: Path, projections: Path, trades: Path,
        equity: Path, signed_receipt: Path, forecast_seal: Path, anti_overfit_report: Path,
    ...
    try:
        hashes = {
            ...
            "anti_overfit_report": _sha(anti_overfit_report),
        }
    except Exception as exc:
        result = {
            "schema": "warroom.v96.blind_proof_run.v1", "market": None,
    ...
    try:
        anti_overfit = _load_json(anti_overfit_report)
        if anti_overfit.get("schema") != "warroom.v96.anti_overfit_report.v1":
            errors.append("anti-overfit report schema mismatch")
        recorded_report_hash = str(anti_overfit.get("report_hash") or "").lower()
        unhashed_report = {k: v for k, v in anti_overfit.items() if k != "report_hash"}
        actual_report_hash = hashlib.sha256(json.dumps(unhashed_report, sort_keys=True, separators=(",", ":"), allow_nan=False, default=str).encode()).hexdigest()
        if recorded_report_hash != actual_report_hash:
            errors.append("anti-overfit report self-hash mismatch")
        if str(anti_overfit.get("market") or "").lower() != market.lower():
            errors.append("anti-overfit market mismatch")
        if anti_overfit.get("historical_blind_proven") is not True:
            errors.append("anti-overfit historical blind proof failed")
        if anti_overfit.get("errors"):
            errors.append("anti-overfit report contains errors")
    except Exception as exc:
        anti_overfit = {"historical_blind_proven": False, "errors": [f"{type(exc).__name__}: {exc}"]}
        errors.append("anti-overfit report unreadable")
    ...
    try:
        ...
        component = f"{market}_bottleneck_price_projection_v96"
        ...
        if bindings.get("anti_overfit_report") != hashes["anti_overfit_report"]:
            errors.append("signed runtime binding mismatch: anti_overfit_report")
        proof = receipt.get("proof") or {}
        if str(proof.get("anti_overfit_report_sha256") or "").lower() != hashes["anti_overfit_report"]:
            errors.append("signed proof hash not bound to anti-overfit report")
    ...
    result = {
        "schema": "warroom.v96.blind_proof_run.v1",
        ...
        "anti_overfit_adjudication": anti_overfit,
...
def main() -> None:
    ...
    for name in ("predictor_manifest", "outcome_manifest", "projections", "trades", "equity", "signed_receipt", "forecast_seal", "anti_overfit_report", "out"):
    ...
    result = run(
        ...
        anti_overfit_report=Path(args.anti_overfit_report),
    )
