...
def run_commodity_native_engine(raw: dict, shared_core: dict, features: dict, macro_board: dict, transmission: dict) -> dict:
    ...
    all_symbols = (raw.get('runtime_universe', {}) or {}).get('commodities') or get_market_ranking_universe(COMMODITY_BUCKETS, COMMODITIES_BACKEND_UNIVERSE)
...
def _setup_row(r, side: str):
    ...
    return {
        ...
        'invalidator': invalidation_from_row(r, side, 'USD spike / curve lemah'),
        'risk': risk_label(r, side, high_vol=0.06),
        'setup_type': 'Execution Focus' if r['symbol'] in FOCUS else ('Family Leader' if side == 'long' else 'Family Weakness'),
...
def _radar_row(r, side: str):
    ...
    return {
        ...
        'signal_quality': signal_quality_label(r),
        'risk': 'Crowded' if side == 'long' else 'Rebound risk',
