...
CHANGED vs original:
- STRUCTURAL_QUAD_CORE_WEIGHTS: flipped to RoC-dominant (growth_momentum > growth_level).
  Hedgeye's entire methodology is built on rate-of-change (RoC), not levels.
  The original had growth_level=0.35 > growth_momentum=0.25 which is anti-Hedgeye.
"""

# --- STRUCTURAL quad weights (slow backbone, quarterly to semi-annual horizon) ---
# Hedgeye is RoC-first: momentum (second derivative) drives regime classification.
# Levels matter for context, but the DIRECTION of change is the primary signal.
STRUCTURAL_QUAD_CORE_WEIGHTS = {
    "growth_level":       0.20,   # was 0.35 — level is secondary context
    "growth_momentum":    0.40,   # was 0.25 — RoC is PRIMARY in Hedgeye
    "inflation_level":    0.10,   # was 0.25
    "inflation_momentum": 0.30,   # was 0.15 — inflation acceleration matters
    "policy_rate":        0.10,
    "liquidity_impulse":  0.10,
}

# --- MONTHLY quad weights (fast overlay, 3-6 week horizon) ---
# Already more momentum-oriented — minor recalibration only.
MONTHLY_QUAD_CORE_WEIGHTS = {
    "growth_level":       0.15,   # was 0.20
    "growth_momentum":    0.50,   # was 0.45 — even more RoC-dominant on monthly
    "inflation_level":    0.10,   # was 0.15
    "inflation_momentum": 0.50,   # unchanged
    "policy_rate":        0.10,
    "liquidity_impulse":  0.10,
