...
NARRATIVE_SPILLOVER: Dict[str, List[Tuple[str, float]]] = {
    "ai_compute": [
        ...
        ("EWT", 0.70), ("EWY", 0.65),  # Taiwan + Korea semis
    ],
    "ai_memory": [
...
MARKET_BREWING_CONFIG: Dict[str, Dict] = {
    ...
    "forex": {
        "min_days": 30,
        "acc_threshold": 0.50,      # FX trend persistence proxy
        "range_threshold": 0.65,      # breakout from consolidation
        "rs_boost": True,             # vs DXY or regional benchmark
        "volatility_filter": True,    # avoid chop
    },
    "commodity": {
        "min_days": 30,
        "acc_threshold": 0.50,
        ...
        "rs_boost": True,
        "volatility_filter": True,    # commodities volatile — need regime filter
    },
    "crypto": {
        "min_days": 30,
        "acc_threshold": 0.50,
        "range_threshold": 0.65,
        "rs_boost": True,             # vs BTC
    ...
    "ihsg": {
        ...
        "acc_threshold": 0.50,
        "range_threshold": 0.65,
        "rs_boost": True,             # vs EIDO or regional
        "volatility_filter": False,
        "foreign_flow_proxy": True,   # use relative vs EIDO as foreign flow signal
    },
    "bonds": {
        "min_days": 30,
        "acc_threshold": 0.45,        # duration momentum
        "range_threshold": 0.60,
        "rs_boost": True,
        "volatility_filter": False,
    },
...
class BottleneckDiscoveryV3:
    ...
    def adaptive_constraint(
        ...
        if market == "ihsg" and qk in ("Q2", "Q1"):
            regime_adj += 0.05  # EM recovery favor
    ...
    def _select_benchmark(self, market: str, prices: Dict) -> Optional[pd.Series]:
        bench_map = {
            "us_equity": "SPY",
            "forex": "DX-Y.NYB",
            "commodity": "GC=F",  # or DBC if available
            "crypto": "BTC-USD",
            "ihsg": "EIDO",
            "bonds": "TLT",
        }
    ...
    def reactive_scan(
        ...
        for ticker, close in prices.items():
            ...
            if market == "ihsg":
                # IHSG foreign flow proxy: relative vs EIDO
            ...
            if cfg.get("volatility_filter") and vol_reg == "extreme":
                continue  # skip extreme volatility = likely news spike, not brewing
