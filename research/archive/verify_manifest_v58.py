"""Strict deterministic package manifest verifier for War Room OS v5.8."""
from __future__ import annotations
import hashlib,json
from pathlib import Path
ROOT=Path(__file__).resolve().parent
MANIFEST=ROOT/'PACKAGE_MANIFEST_V58.json'
IGNORE_DIRS={'.git','.venv','__pycache__','.cache','.pytest_cache','audit_logs','runtime','audit_state_v55','audit_state_v56','audit_state_v57','audit_state_v58'}
IGNORE_EXACT={
 'PACKAGE_MANIFEST_V58.json','PACKAGE_MANIFEST_V57.json','PACKAGE_MANIFEST_V56.json','PACKAGE_MANIFEST_V55.json','PACKAGE_MANIFEST_V53.json',
 'V55_USER_VALIDATION_REPORT.json','V56_USER_VALIDATION_REPORT.json','V57_USER_VALIDATION_REPORT.json','V58_USER_VALIDATION_REPORT.json',
 ...
 # Regenerated test/audit outputs excluded from immutable release boundary.
 'V52_HARDENING_ADVERSARIAL_REPORT.json','V52_BUNDLED_DATA_INTEGRITY_REPORT.json','V55_PARQUET_COMPAT_VALIDATION.json',
 'V70_OPTIONS_GAMMA_VALIDATION.json','V71_OPTIONS_PROSPECTIVE_VALIDATION.json','V72_SIGNED_DEALER_VALIDATION.json',
...
def verify(root:Path=ROOT,manifest_path:Path|None=None)->dict:
 ...
 if raw.get('schema')!='warroom.package_manifest.v58':res['errors'].append('manifest schema mismatch')
