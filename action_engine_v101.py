"""Operational non-technical action engine for War Room OS V10.1.
...
from carry_trade_engine_v101 import pair_state, build_carry_book
...
def action_for_packet(packet: Mapping[str, Any], current: Mapping[str, Any], policy: Mapping[str, Any], peers: Mapping[str, list[dict[str, float]]], macro: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if market in {"us", "idx"}:
        ...
        relative_score, details = _fx_score(ticker, current, macro)
        carry = pair_state(ticker, current, macro)
        carry_score = _finite(carry.get("directional_score"))
        score = _mean([relative_score, carry_score])
        projection = _scenario_projection(price, score, market)
        funding_pos = _finite(((carry.get("funding_positioning") or {}).get("leveraged_net_share"))) if isinstance(carry.get("funding_positioning"), Mapping) else None
        target_pos = _finite(((carry.get("target_positioning") or {}).get("leveraged_net_share"))) if isinstance(carry.get("target_positioning"), Mapping) else None
        positioning_direction = None
        if carry_score is not None and (funding_pos is not None or target_pos is not None):
            raw_pos = (target_pos or 0.0) - (funding_pos or 0.0)
            positioning_direction = _clip(raw_pos / 0.30) * (1.0 if carry_score >= 0 else -1.0)
        causal = 0.45 if carry.get("state") in {"CARRY_ON", "SELECTIVE_CARRY"} else 0.20 if carry.get("state") == "CROWDED_CARRY" else -0.45 if carry.get("state") in {"UNWIND_RISK", "UNWIND_ACTIVE"} else 0.0
        components = {"fundamental": _mean([relative_score, carry_score]), "valuation": _signed_scale(_finite(projection.get("expected_return")), 0.12) if projection.get("valid") else None, "causal": causal, "macro": relative_score, "flow": positioning_direction}
        inputs["relative_macro"] = details
        inputs["carry_trade"] = carry
    ...
    return {
        "schema": "warroom.v101.current_action.v1", "generated_at": dt.datetime.now(UTC).isoformat(timespec="seconds").replace("+00:00", "Z"),
...
def load_policy(path: Path = HERE / "V101_ACTION_POLICY.json") -> dict[str, Any]:
...
def enrich_packets(packets: Mapping[str, Mapping[str, Any]], current: Mapping[str, Any], policy: Mapping[str, Any] | None = None) -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    for market in MARKETS:
        for ticker, raw in (packets.get(market) or {}).items():
            ...
            packet["schema"] = "warroom.v101.unified_decision_packet.v1"
            packet["current_action"] = action
            if market == "fx": packet["carry_trade"] = ((action.get("inputs") or {}).get("carry_trade") or {})
            packet["decision"] = {
    ...
    alpha = {
        "schema": "warroom.v101.action_center.v1", "state": "SHADOW_CANDIDATES_AVAILABLE" if shadow else "RESEARCH_ACTIONS_AVAILABLE",
    ...
    return output, {"alpha_center": alpha, "macro_state": macro, "carry_trade": carry_book, "peer_counts": {m: len(v) for m, v in peers.items()}}
