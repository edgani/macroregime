"""Machine-readable V9.9 unified decision and execution readiness audit."""
from __future__ import annotations
import datetime as dt
import hashlib, json
from pathlib import Path
from typing import Any
from execution_reconciliation_v99 import verify_ledger
...
def audit()->dict[str,Any]:
    policy=_load(HERE/'V99_LIMITED_PRODUCTION_POLICY.json',{})
    universe=_load(HERE/'V99_EXECUTION_REFERENCE_UNIVERSE.json',{})
    registry=_load(HERE/'component_registry_v99.json',{'components':{}})
    quotes=_load(HERE/'runtime'/'v99_trading'/'execution_quotes.json',{})
    ...
    required=['dashboard.html','decision_packet_v99.py','public_snapshot_reader_v98.py','trading_control_plane_v99.py','execution_quote_collector_v99.py','execution_reconciliation_v99.py','V99_LIMITED_PRODUCTION_POLICY.json','V99_EXECUTION_REFERENCE_UNIVERSE.json','component_registry_v99.json']
    ...
    payload={
        'schema':'warroom.v99.trading_readiness.v1','release':'War Room OS V9.9 Unified Decision Packet',
        ...
        'kill_switch_engaged':(HERE/'runtime'/'v99_trading'/'KILL_SWITCH.json').is_file(),
        ...
        'claim_limit':'V9.9 unifies research and execution context. It does not manufacture alpha and exports no order without exact proof, a valid ticker value bridge, fresh quote, risk checks and human approval.',
    }
...
def write(path:Path|None=None)->dict[str,Any]:
    result=audit(); out=path or (HERE/'runtime'/'v99_trading'/'V99_TRADING_READINESS.json'); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(result,indent=2),encoding='utf-8'); return result
