...
Measures divergence between price momentum and credit/fundamental reality.
"""
...
def run_reflexivity(prices: Dict[str, pd.Series], fred: Dict[str, pd.Series], quad: str = "Q3") -> Dict:
    ...
    for t in ["SPY", "QQQ", "IWM", "GLD", "TLT", "BTC-USD", "ETH-USD"]:
        ...
        scores[t] = {
            "momentum_1m": round(mom, 4), "momentum_3m": round(mom_3m, 4),
            "reflexivity_score": round(score, 2),
            "divergence": "PRICE_AHEAD" if score > 0.3 else "FUNDAMENT_AHEAD" if score < -0.3 else "ALIGNED",
        }
    return {
        "super_bubble_score": round(bubble_score, 1), "super_bubble_max": 10,
        "credit_conditions": credit, "avg_equity_momentum_3m": round(avg_mom, 4),
        "divergence_index": round(divergence, 2), "stage": stage, "stage_color": stage_color,
        "stage_desc": {
