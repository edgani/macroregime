...
def page_dashboard_control_tower(snap:Dict)->None:
    ...
    q=snap['q']; f=snap['f']; rot=snap['rotation']; family=snap.get('family','reflation')
    render_hero_cards_refined(snap)
    st.markdown('---')
    render_flow_state_strip(q)
    render_causal_workflow_board(snap)
    render_master_rotation_graph(q, f, rot, family)
    render_rotational_flow_map(q, rot, f, family)
    drivers=snap.get('top_drivers',[]) or []
    if drivers:
        render_top_drivers_now(drivers, title='🧠 Why This Is Moving')
    sh('🌐 MARKET BRANCHES — klik tab market untuk ticker & dampak detail')
    render_market_branch_cards(snap)
    st.markdown('---')
...
1. **Dashboard** — lihat kondisi board, jalur utama, dan market yang terdampak
2. **Market tab** — lihat siapa yang diuntungkan / dirugikan dan ticker action
