...
BOOT = {
    ...
    "runtime": {
        ...
        "snapshot_sequence": 1, "content_hash": "boot-v28-test",
    },
...
def main() -> int:
    ...
    with preserve_runtime():
        ...
        try:
            ...
            research = snap.get("research_kernel") or {}
            research_markets = research.get("markets") or {}
            ok = all(counts[name] > 0 for name in counts) and len(research_markets) == 5 and research.get("global_permission") == "CAPITAL_BLOCKED"
            report["tests"].append(
                {
                    ...
                    "counts": counts,
                    "research_adapters": sorted(research_markets),
                    "capital_permission": research.get("global_permission"),
                }
            )
        finally:
            stop(proc)
        ...
        try:
            ...
            research = snap.get("research_kernel") or {}
            ok = state == "NO_DATA" and str((snap.get("meta") or {}).get("source") or "") != "INITIALIZING" and research.get("global_permission") == "CAPITAL_BLOCKED"
            report["tests"].append(
                {
    ...
    output = HERE / "V32_STARTUP_VALIDATION_REPORT.json"
