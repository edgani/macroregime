"""Fail-closed five-market promotion summary for War Room OS V9.8."""
...
def evaluate_all(runs: dict[str,dict[str,Any]]) -> dict[str,Any]:
    ...
    for market in MARKETS:
        ...
        results[market]=run
        if run.get("schema") not in {"warroom.v96.blind_proof_run.v1","warroom.v97.blind_proof_run.v1","warroom.v98.blind_proof_run.v1"}: reasons.append(f"{market}: wrong proof-run schema")
    ...
    payload={"schema":"warroom.v98.global_market_adjudication.v1","global_trading_ready":not reasons,"capital_permission":"ALL_MARKET_LIMITED_PRODUCTION_ELIGIBLE" if not reasons else "BLOCKED","market_results":results,"reasons":sorted(set(reasons))}
